...
7. 시총 상위 200개의 7·14·21일 정량 점수를 계산해 복합 상위 50개를 고른 뒤 KIS 현재가·분봉으로 신선도와 가격을 교차검증한다.
...
AI는 정량 상위 50개 밖의 종목을 추가하거나 원시 숫자를 수정할 수 없다. 실시간 손절·익절은 외부 AI를 기다리지 않고 로컬 엔진이 실행한다. KIS 공식 LLM 예제·전략 빌더·백테스터는 참고 구현으로 사용하되, 프로젝트의 사용자 승인 규칙과 -7% 손절 정책을 대체하지 않는다.
