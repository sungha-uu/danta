...
`D:\WorkSpace\Codex\financial_statement_analysis-main`의 최신 전 종목 산출물에는 943개 KOSPI 종목의 코드·이름·업종·현재가·시가총액·시총구분·7일 평균 거래대금·7/14/21일 수익률·14일 투자자 수급·TTM 손익과 재무 품질이 있다. 다음 용도로 재사용할 수 있다.

- 최초 사전필터 시나리오 산정과 1분봉 백필 대상 축소
- 시가총액·업종·재무 위험·TTM 결측의 보조 스냅샷
- 기존 7·14·21일 기간수익률·수급의 초기 대시보드 참고값

다음 값은 별도 확인이 필요해 기존 Excel만으로 확정하지 않는다.

- 거래정지·관리·투자경고/위험·상장폐지·정확한 보통주 구분과 상장일
- 최근 거래일별 거래대금 지속성
- 실시간 스프레드·호가 깊이·예상 슬리피지
- 1분봉 원본과 60분봉 구조 지표
- 행별 가격·수급의 정확한 `data_as_of`

장기 연동은 표시용 Excel 시트명에 직접 결합하지 않는다. 권장 순서는 다음과 같다.

1. 최신 Excel을 스키마·생성일·행수·종목코드 6자 영문대문자·숫자 형식·중복 검증 후 최초 부트스트랩에 한해 읽는다.
2. 기존 프로젝트가 안정된 공개 계약의 `universe_snapshot.json` 또는 Parquet을 함께 출력하도록 한다.
3. Danta는 이를 읽기 전용 보조 공급자로 가져오되 KRX 종목상태와 KIS 거래 직전 값을 최종 검증한다.
4. 원본 파일은 복사·수정하지 않고 공급자·기준시각·입력 해시를 저장한다.

기존 분석기의 핵심 실행부가 보존된 컴파일 바이트코드 래퍼이므로 Danta가 해당 내부 모듈을 직접 import하지 않는다. 산출 데이터 계약으로만 연결해 장애와 버전 결합을 격리한다.

후보 30개 생성을 위해 전 종목을 장중 KIS REST로 무제한 반복 조회하지 않는다. KRX 일괄 자료는 종목군·유동성·수급·시장 국면의 사전필터에 사용하고, 박스 구조는 저장된 정규장 1분봉에서 계산한다. KIS는 공식 분봉 조회의 증분 수집·교차검증과 지정 종목 실시간 감시, 계좌·주문에 우선 사용한다.
...
5. 확보된 7·14·21거래일 박스·진폭·하단 접촉·하단+10% 목표 도달·효율성·포함률을 60분봉 OHLCV와 원본 1분봉 순서로 계산한다. 일봉 종가를 구조 산식의 폴백으로 사용하지 않는다.
6. KRX 투자자별 순매수로 개인·외국인·기관·금융투자·연기금의 가용 기간 합계를 계산한다.
7. 정량 점수로 30개를 고른 뒤 KIS 현재가·분봉으로 신선도와 가격을 교차검증한다.
