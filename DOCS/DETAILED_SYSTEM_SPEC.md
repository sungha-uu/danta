...
3. 시총 상위 200개를 14일 반복 상승폭·횟수로 정렬하고, 상위 50개에 AI 종합 코멘트를 부여한다.
...
- 사용자는 종목·진입 목표가·주문가능현금 배정률·알고리즘 버전을 `ENTRY_MANDATE`로 승인한다. 시스템은 현재 잠긴 계좌 모드와 그 범위 안에서 정량 신호를 결합해 진입 시점을 결정하고 매수 주문을 실행한다. 박스 이탈은 참고 신호이며 승인을 취소하지 않는다. 종목 선택만으로는 매수 승인이 성립하지 않는다.
...
| 매수 실행 | 승인 메시지에서 종목·목표가·사용자 배정률·정책 버전을 검증하고 KIS 주문가능현금을 조회한 뒤 최대 정수 수량 주문을 제출한다. |
...
| P0 | 일별 코스피 전체 스크리닝 | 시총 상위 200개 7·14·21일 정량 지표, 상위 50개 AI 종합 코멘트 |
...
7. 붕괴 위험, 결측, 이벤트 위험을 반영해 정량 상위 50개의 AI 종합 코멘트 생성
...
16:50 정량 후보 200개와 상위 50개 AI 종합 코멘트 저장
...
- 이 cooldown은 중복 알림에만 적용하며 손절 종목의 매매 재진입 금지와 무관하다.
...
| 지표 | 정의 |
| --- | --- |
| Comment cohort precision | 정량 순위 구간·AI 코멘트 주제별 다음 5일 반등 비율 |
...
- 구조 적합도와 규칙 기반 유지 확률
- 정량 후보 200개 산출과 상위 50개 AI 종합 코멘트 배치
- FastAPI 조회 API
...
- 장 마감 후 정량 후보 200개와 상위 50개 종목의 AI 종합 코멘트가 자동 생성된다.
