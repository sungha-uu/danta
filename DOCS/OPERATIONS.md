...
- DB 저장: 매 표본
- 상태전환 메일: 최초 비정상 상태, 위험 단계 상향, 정상 복귀와 단계 완화를
  포함한 모든 확정 상태전환에 발송한다. 동일 상태를 유지하는 표본에는 발송하지
  않는다.
- 시장 상태 SMTP의 단일 소유자는 독립 `market-monitor` 프로세스다. 주문
  런타임은 같은 시장 위험을 신규매수 차단과 감사 로그에 적용하지만
  중복 시장 메일을 발송하지 않는다.
