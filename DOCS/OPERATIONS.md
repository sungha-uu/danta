...
정량 순위와 무관하게 해당 거래일 배치에 반드시 포함하라는 사용자 명시 승인이 있으면
`--require-include`를 추가한다. 데이터·시장·계좌·진입 안전 게이트는 우회하지 않는다.

```powershell
.\.venv\Scripts\danta.exe autonomy prefer --symbols 000660 --trading-date 2026-08-07 --require-include --execute
```
...
프로세스 존재와 `RUNNING`만으로 정상이라고 보고하지 않는다. 사용자에게 정상 여부를
답하기 전에 다음을 모두 확인한다.

1. 런타임 heartbeat와 단일 worker, 활성 명령·포지션·미체결 주문
2. 자율 캠페인 만료·kill switch·일일 배치 한도
3. 직전 완료 거래일 기준 AI 보고서 신선도와 200종목 오버레이 신선도·커버리지
4. 시장 상태와 영속 재개 잠금
5. 사용자 우선 종목의 적용일·선정 정책·현재 후보 자격
6. 최근 `AUTONOMOUS_ENTRY_BLOCKED/ERROR`, 종목·지정가·체결 메일 생성·발송 상태

하나라도 실패하면 `정상`이 아니라 정확한 차단 사유와 사용자 조치 필요 여부를
보고한다.
...
매 거래일 정규장 종료 직후 `15:31 Asia/Seoul`에 `Danta-Close-Prefetch-1531`이 KIS 일봉으로 거래일을 확인한 뒤 기존 시총 상위 200개의 최종 1분봉을 증분 수집하고 장 마감 오버레이를 즉시 게시한다. 이 경로는 KRX 웹 인증 장애와 분리해 공식 KRX 종가·차트·장중 정량값이 장중 마지막 게시값으로 남지 않게 한다. `16:00`에는 `Danta-Daily-1600`이 KRX 당일 데이터와 변경된 시총 상위 200개를 확정하고 재무·수급·후보·대시보드를 갱신한다. 16:00 데이터가 아직 준비되지 않았거나 네트워크가 일시 실패하면 16:10·16:20·16:30에 같은 일자를 재시도한다. 해당 거래일 성공 표식이 있으면 뒤의 재시도는 아무 작업 없이 종료한다. 토·일요일에는 실행하지 않고, 평일 휴장일은 KIS 일봉 또는 KRX 최신 거래일이 당일과 다르면 게시하지 않는다.
...
| 등급 | 사건 | 정책 |
| --- | --- | --- |
| `INFO` | 정량 후보 리포트·AI 종합 코멘트·장 마감 | 1회 |
...
Pages는 공식 주문 후보 200개의 정량 정보와 상위 50개의 AI 종합 코멘트·수급·박스 근거를 보여주는 정적 분석 프런트엔드일 뿐 주문·계좌 서버가 아니다.
...
자율 캠페인은 공개 Pages JSON을 주문 원본으로 사용하지 않는다. 16:00 기준
보고서는 종목·정량 순위·AI 종합 코멘트·완결된 7·14·21일 구조를 제공한다. KRX 정규장에는 200종목의
당일 1분봉 증가분을 30분마다 수집해 14일 장중 순위를
`private/prod/intraday_candidate_overlay.json`에 원자적으로 저장한다. 180종목 이상,
관측 후 35분 이내, 기준 보고서 일치 조건을 통과한 revision만 자율 선정에 사용한다.
전일 기준 보고서가 직전 평일보다 오래되면 `REPORT_STALE`로 신규매수를 차단한다.
공개 대시보드는 정적 기준 보고서로 유지하고 주문 판단용 장중 오버레이는 비공개로
유지한다.

공개 후보 Pages에는 민감정보를 제거한 `intraday_overlay.json`만 1시간마다 커밋한다.
정적 JavaScript가 5분마다 새 파일을 확인해 14일 현재가·장중 순위·반복 상승 지표와
차트를 갱신한다. 화면의 상단 기준시각에 `장중 MM.DD HH:MM`을 함께 표시한다.
수급·뉴스·AI 코멘트는 완결 보고서 값이며 장중 오버레이가 덮어쓰지 않는다.

실계좌 자율 캠페인은 최대 동시 3종목, 하루 최대 3배치다. 기존 활성 배치가 완전히
청산되고 KIS 잔고·미체결·로컬 상태가 모두 평탄할 때만 다음 장중 revision으로
재선정한다. 장중 수집은 보호 감시와 별도 비동기 작업이며 종목별 REST 요청 사이에
0.10초를 양보해 손절·익절·주문대조가 우선 실행되도록 한다.
...
- 당시 산출물 검사(폐기된 등급 체계): 활성 구조 무효화선 아래 추천 0개, `LOW` 신뢰 적극 추천 0개.
- 당시 실제 브라우저 검사(폐기된 등급 체계): 후보 50개, 에이전트 등급, 뉴스 링크 99개, 토론 링크 50개를 확인했다.
...
- 당시 7일 에이전트 등급(폐기된 체계): 적극 추천 4, 추천 8, 비추천 30, 적극 비추천 8
...
- AI 미검토: 51~200위 150개는 AI 종합 코멘트·뉴스·토론 요약을 생성하지 않음
...
- 정렬 대상은 `상승세·반복 상승폭·반복 횟수·외국인+기관 수급강도·정량점수` 다섯 열로 제한했다.
...
- 자동매매 SMTP는 `자율매매 종목 선정완료.`, `지정가격 산정완료.`,
  `자율매매 신규진입 보류.`, `자동매수 체결완료.`,
...
장외·휴장 시간의 시장 전체 데이터 미제공은 정상적인 세션 공백으로 취급해 영구
재개 잠금을 만들지 않는다. 정규장 중 시장 센싱 오류 또는 확정된 `RISK_OFF/PANIC`만
잠금을 만들며, 자율매매가 이 사유로 중단되면 시장 상태 전환 메일과 별개로 거래일당
한 번 `자율매매 신규진입 보류.` 메일을 보낸다. 이 메일은 신규 종목·지정가가 생성되지
않은 이유를 알리는 자동매매 운영 알림이다.

런타임 시작 시 시장 위험 기본값을 정상으로 간주하지 않는다. 첫 유효 시장 표본이
코어에 반영될 때까지 신규 후보 생성을 차단하고, 후보 실시간 시세 조회가 끝난 뒤에도
명령함 제출 직전에 시장 상태와 재개 래치를 다시 검증한다.
...
대시보드는 `4 후보 danta_report`, `5 시장 danta_market_status`, `6 공개용
자율매매 실적`, `7 AI 종목 추천 및 재무제표`로 분리하고 1번 통합 운영·안전 코어의 관제 Pages에서 연결한다. 6번에는 최초 투자금, 현재 투자금, 실현·평가·누적 손익, 수익률과
정제된 매수·매도 이력만 허용한다. 계좌번호 일부나 증권사명만 화면에서 숨기는
블랙리스트 방식은 사용하지 않고, 공개 전용 DTO가 허용한 필드만 생성물에 넣는다.
계좌번호·계좌별칭·증권사·주문번호·승인문·수량 산출 근거·주문가능현금·원본 KIS
응답은 HTML, JSON, 소스맵, 메타데이터와 Git 커밋 메시지 모두에서 금지한다.

통합 운영 Pages는 상태 색상, 마지막 정상 실행, 다음 예약 실행, 장애 시작,
최근 오류 한 줄, 현재 작업, 보호매도·신규매수·시장 재개 래치, 이메일·Pages 배포,
최근 조치와 3~7번 공개 화면 링크를 표시한다. 모든 상태는 Python 헬스체크 결과에서 생성하고
Codex가 정상으로 수동 기입하지 않는다. `Codex 전달문 복사`는 분석 요청만 만들며
주문 승인 권한을 갖지 않는다.
...
- 번호 체계: `1 통합 운영·안전 코어`, `2 자율매매`, `3 시장 센싱`,
  `4 단기 후보 대시보드`, `5 KOSPI 시장·자금 흐름 대시보드`,
  `6 공개용 자율매매 실적 대시보드`, `7 AI 종목 추천 및 재무제표 대시보드`
