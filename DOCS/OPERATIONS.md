...
2026-07-26에는 KRX 실제 데이터로 2026-07-24 기준 후보 30개 보고서를 생성했다. KIS 모의 서버의 후보 30개 현재가 교차검증까지 통과해 산식 버전은 `box-quant-v1+kis-quote-verified`다. 모델 단계는 `quant-baseline-no-llm`이다. 가격·거래량·거래대금·투자자 수급은 실제 데이터이며 뉴스·공시·AI 정성 검토는 아직 미연결임을 화면에 명시한다.
