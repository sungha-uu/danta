...
3. `박스 하단 접촉 후 3거래일 이내 +10% 도달` 자격 게이트를 통과한 공식 후보를 최대 30개 생성
...
Pages는 자격 게이트를 통과한 공식 후보 최대 30개 전체의 AI 등급·코멘트·수급·박스·자격 근거를 보여주는 정적 분석 프런트엔드일 뿐 주문·계좌 서버가 아니다.
...
- 적용 정책: `exit-defense-profit-paper-v2-minus5-floor`
...
- DB 저장: 매 표본
- 상태전환 메일: 최초 비정상 상태 및 `NORMAL/CAUTION/RISK_OFF/PANIC` 전환 시만 발송
...
시장 데이터 수집 오류가 발생하면 신규매수는 `RISK_OFF`로 막고 종목 실시간 감시, 주문 체결 대조, -3%·-5% 방어와 -7% 강제손절은 계속 실행한다.
...
후보 대시보드 `danta_report`와 시장 현황판은 갱신 주기·장애 영향·독자 목적이 다르므로 저장소와 Pages를 합치지 않는다.
