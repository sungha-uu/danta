...
| 2 | 자율매매 | 후보 오버레이·진입·OMS·손절/익절·포지션 복구 |
| 3 | 시장 센싱 | `MarketWideCollector`·`MarketWideMonitor`·`MarketRegimeGuard` |
...
| 7 | AI 종목 추천 및 재무제표 | 독립 `financial_statement_analysis_daily_report` Pages의 링크·갱신 상태 관제; Danta 주문 입력 의존 금지 |

1번 코어의 운영 화면은 1~7번 헬스 상태를 모으는 관제 허브다. 각 행에는
`NORMAL/WARNING/FAILED/STOPPED/NOT_IMPLEMENTED`, 마지막 정상 시각, 다음 실행 시각,
장애 시작 시각, 최근 오류 요약, 현재 작업, 보호매도·신규매수 상태, 시장 재개 확인
필요 여부, 이메일·Pages 배포 상태와 최근 조치 이력을 포함하고 3~7번의 공개 화면으로
...
| 후보·AI 엔진 | 시총 상위 200개 계산·전체 주문 선택, 정량 상위 50개 AI 심층검토와 버전 저장 |
| 운영 오케스트레이터 | [`TRADING_ORCHESTRATOR.md`](TRADING_ORCHESTRATOR.md)에 따라 종목별 actor 생성·복구·중지, 계좌 자금 예약, 주문 우선순위와 진입/보호 작업 분리 |
| 통합 실시간 모니터 | 하나의 장기 실행 프로세스가 현재 승인된 최대 3개, 향후 검증된 `N`개 종목의 KRX·NXT·예상체결가 입력을 시간대별로 전환하고 같은 actor·포지션·주문 큐를 유지 |
...
| 정적 리포트 빌더 | 공식 주문 후보 200개를 검증하고 7·14·21일 지표와 자체 차트 생성 |
...
STOP_CONTEXT_ASSESSMENT_REQUESTED, SYMBOL_REQUALIFICATION_REQUESTED,
...
정규장 감시와 NXT 감시는 별도 운영 프로세스가 아니다. 단일
`UnifiedTradingMonitor`가 다음 상태를 순환하며 공급자 스트림만 교체한다.

| KST 평일 기준 | 상태 | 주 입력 | 주문 권한 |
| --- | --- | --- | --- |
| 08:00~08:30 | `NXT_PREMARKET` | NXT 체결·호가 | 신규매수·즉시매도 없음 |
| 08:30~08:50 | `NXT_WITH_KRX_EXPECTED` | NXT + KRX 예상체결가 | 보호계획 평가만 |
| 08:50~09:00 | `OPENING_PLAN_LOCKED` | NXT + KRX 예상체결가 | 09시 보호계획 잠금, 주문 없음 |
| 09:00~15:30 | `KRX_REGULAR` | KRX 체결·호가, 시장 전체 위험 | 승인 매수·자동매도 |
| 15:30~20:00 | `NXT_AFTERMARKET` | NXT 체결·호가 | 보유 위험 관찰, 다음 장 대응자료 |
| 그 외·휴장 | `DORMANT` | heartbeat·잔고/연결 준비 | 주문 없음 |

- 사용자가 어느 시간에 `ENTRY_MANDATE`를 전달해도 같은 런타임이 승인 상태를
  보존한다. 신규매수는 KRX 정규장과 승인된 주문 가능 조건이 동시에 성립할
  때만 제출한다.
- NXT 이벤트는 장외 관찰·익일 보호계획 입력이며, 정규장 주문 엔진에 그대로
  흘려 장외 주문을 우발적으로 만들지 않는다.
- 09시 전환 시 KIS 잔고·미체결을 다시 대조한 뒤 잠긴 보호계획을 같은
  `TradingOrchestrator` P0/P1 큐에 전달한다.
- 세션 전환은 프로세스 재시작이 아니다. 종목 `generation`, 평균체결가,
  최고수익률, 자금 예약과 주문 멱등키를 그대로 유지한다.
- WebSocket 전환·재연결에 실패하면 신규매수만 차단하고 REST 손실 방어,
  잔고 대조와 열린 포지션 보호 책임은 유지한다.
- 08:30~09:00의 NXT 체결·호가와 KRX 예상체결가는 같은 KIS 승인키를 사용하는
  단일 WebSocket 연결에 함께 구독한다. 같은 앱키로 두 개의 병렬 WebSocket을
  만들지 않으며, 공급자 오류는 하위 예외 유형·메시지까지 감사 로그에 남긴다.
