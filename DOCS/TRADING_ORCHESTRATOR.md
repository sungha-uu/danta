...
- 캠페인 컨트롤러는 계좌가 완전 유휴 상태일 때만 새 배치를 만들며 동일
  `report_data_as_of`에 대해 한 번만 제출한다. 기존 수동 명령이나 열린 포지션을
  덮어쓰거나 섞지 않는다.
...
`trading-runtime`은 `--recover` 같은 별도 사용자 옵션을 두지 않는다. 모든
프로세스 시작에서 다음 순서를 강제한다.

1. 단일 인스턴스 잠금을 얻고 주문 제출 게이트를 닫는다.
2. DB의 열린 포지션·미완료 주문·부분체결·자금 예약·활성 승인문을 읽는다.
3. KIS 잔고·미체결·관련 거래일 주문·체결을 조회한다.
4. KIS 주문번호를 우선키로 로컬 주문과 연결하고 종료 중 체결을 반영한다.
5. `SymbolSession`, `ManagedPosition`, 주문번호 매핑, 누적체결, 자금 예약과
   익절 최고수익률을 복원한다.
6. 불일치가 없으면 현재 시간의 통합 거래 세션으로 전환한다.
7. 주문 성공과 로컬 저장 사이에 종료되어 결과가 하나로 확정되지 않으면
   재주문하지 않고 `QUARANTINED/ENTRY_BLOCKED`로 둔다. 실제 보유수량의
   보호 감시는 계속한다.

프로세스는 계좌 단위로 실행하고 승인문은 실행 중 수신한다. 활성 승인문이
없어도 KIS에 보유 포지션이 있으면 보호 actor를 구성한다.
...
private/commands/
  inbox/       # 작성 완료된 새 명령
  active/      # 검증·DB 반영 후 진행 중
  archive/     # COMPLETED/CANCELLED/REJECTED
private/runtime_state.json
```

- 작성자는 같은 디렉터리의 임시파일을 완성한 뒤 원자적 `replace`로
  `inbox/<command_id>.json`을 만든다.
- 런타임은 파일 변경 이벤트를 우선 사용하고 5초 폴링으로 유실을 보완한다.
- 같은 `command_id`는 한 번만 수락하며 처리 중 새 승인문은 명시적으로
  거부하거나 큐잉한다. 현재 정책은 활성 승인문 하나, 최대 3종목이다.
- 파일은 명령 전달 수단이지 체결·포지션의 권위 원본이 아니다.
- 전 종목이 `CLOSED/INVALIDATED`이고 미체결·예약이 없을 때만
  `COMPLETED`로 보관한다.
...
모든 시작은 ACTIVE 승인문을 곧바로 재활성화하지 않는다. 선택 종목 전부에 대해
KIS 잔고 0, KIS 미체결 0, 승인 접수 이후 DB 포지션 CLOSED가 함께 확인되면 해당
승인 생명주기는 이미 끝난 것으로 판정해 `COMPLETED`로 보관한다. 세 증거 중 하나라도
부족하면 완료로 추정하지 않으며 기존 복구·격리 절차를 따른다. 브로커 조회 실패 때는
신규 진입보다 중단을 우선한다.
