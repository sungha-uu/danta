...
- [ ] 기간수익률은 선택 기간 첫 거래일 수정종가 대비 현재가 변화율이며 구조 점수 입력과 혼용하지 않는다.
- [ ] 단일 종합표에 후보 30개 전부와 AI 4단계 등급·코멘트·위험·무효화 조건이 있으며 등급별 개수를 강제로 맞추지 않는다.
- [ ] 대시보드는 공식 후보 30개와 31~50위 확장 관찰군을 구분해 총 50개를 표시하고, 확장 관찰군의 선택·주문 승인을 차단한다.
...
- [ ] 현재가→기간 고점은 과거 가격 공간으로 표시하며 예측 확률이나 보장 수익률로 표현하지 않는다.
