...
- [ ] 왕복 횟수는 각 7·14·21일 가격창에서 독립 계산되고 모든 기간에 동일한 고정 예시값을 재사용하지 않는다.
- [ ] 하단→상단 또는 상단→하단 편도는 왕복으로 세지 않고, 하단→상단→하단 또는 상단→하단→상단에서만 완전 왕복 1회로 계산한다.
- [ ] 한 60분봉이 상·하단을 모두 접촉했을 때 원본 1분봉 선후관계 검증 없이는 왕복을 가산하지 않는다.
