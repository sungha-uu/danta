...
- [ ] 대시보드 7·14·21일 전환 시 순위·AI 분석 30개·수익률·박스·차트·거래대금·거래량·수급이 모두 같은 기간을 사용한다.
- [ ] `READY` 기간수익률은 팝업 차트 첫 60분봉 종가 대비 현재가 변화율과 일치하고, `WARMING_UP` 참고 수익률은 첫 거래일 수정종가 기준임을 구분하며 구조 점수 입력과 혼용하지 않는다.
- [ ] 단일 종합표에는 +10% 자격 게이트를 통과한 공식 후보만 최대 30개 표시하고, 각 후보의 AI 4단계 등급·코멘트·위험·무효화 조건을 제공하며 등급별 개수를 강제로 맞추지 않는다.
- [ ] 공개 보고서가 7일 정량 상위 50개를 추천 여부와 무관하게 모두 포함하며 기본 화면에서 임의로 숨기지 않는다.
...
- [ ] 하단이 첫 거래일 대비 8% 이상 낮아진 종목은 낙하 중 저가로 분류되어 추천 등급이 제한된다.
...
- [ ] 최초 백필은 사전필터 통과 종목의 정확한 7거래일을 수집하고 중단 후 종목·거래일 체크포인트에서 재개된다.
