...
### 2026-08-07 후보 탐색과 주문 실행 분리 검증

- [x] 시장 `CAUTION`에서도 200종목 재평가와 다음 후보·감시 지정가 계산은 계속된다.
- [x] 기존 활성 명령 또는 보유 포지션이 있어도 다음 후보 대기 스냅샷이 갱신된다.
- [x] 보유·진행 주문 종목을 합쳐 남은 슬롯만큼만 다음 후보를 지정하며 현재 보유
  종목은 중복 선정하지 않는다.
- [x] 위 상태에서는 신규 주문 명령이 생성되지 않는다.
- [x] 후보 종목 구성이 같으면 주기적 재평가가 선정·지정가 메일을 반복하지 않는다.
- [x] 후보 종목 구성이 달라지면 새 선정·지정가 메일을 발송한다.
- [x] 대기 스냅샷에는 AI 등급을 기록하지 않고 정량 선정 근거와 실행 차단 사유만
  기록한다.
- [x] 상위 50개만 수급 `READY`, 나머지 150개가 `UNAVAILABLE/null`이어도 200종목
  오버레이 완결성을 인정하고 수급 `READY` 후보만 주문 평가에 사용한다.
- [x] 재시작 직후 시장 첫 표본 전에는 활성 명령이 있어도 신규 진입 주문을 만들지
  않으며 기존 보유 포지션의 보호 감시는 유지한다.
...
- [ ] 해결 표본이 부족한 `LOW` 신뢰도는 긍정 의견으로 단정되지 않고 화면에 후향 관측값임을 명시한다.
...
- [ ] 동일 보고서·동일 장중 오버레이 revision의 자율 배치가 재시작·중복 폴링에도
  한 번만 생성되고, 다른 revision도 일일 3배치를 초과하지 않는다.
- [ ] 자율 배치는 직전 거래일 16:00 기준 보고서만 사용하며 오래된 보고서는
  `REPORT_STALE`로 신규매수 0건이다.
- [ ] 200종목 장중 오버레이는 관측 후 35분 이내, 고유 종목 180개 이상, 기준 보고서
  일치 조건을 통과하고 관측시각·revision·실시간가·14일 순위를 기록한다.
- [x] 외국인+기관 장중 추정 순유입은 가점이지만 절대 통과 조건이 아니며, 약한
  순유출 종목도 가격 할인 폭·반복 상승 기회가 충분하면 점수로 비교된다. 초기
  `-3.00%` 이하 심각한 순유출과 수급 미수집만 신규진입을 차단한다.
  (2026-08-07: 가중치·심각 유출 경계·약한 음수 허용 회귀 테스트 통과)
- [ ] 장중 오버레이 수집 실패·KIS 제한·부분 커버리지는 신규매수만 차단하며
  보유종목 보호 감시·강제손절·익절·체결대조 지연을 만들지 않는다.
- [ ] 공개 오버레이에는 계좌·주문·승인문·API 비밀정보가 없고, Pages에서 200행,
  장중 기준시각, 장중 1위·현재가가 원본 오버레이와 일치하며 콘솔 오류가 0건이다.
- [ ] 활성 배치가 일부만 청산된 경우 새 종목을 채우지 않고, 계좌·명령·미체결이
  완전히 평탄한 경우에만 다음 revision 배치를 만든다.
...
- [ ] 자율 배치 제출 직후 `자율 모의투자 종목 선정완료.` 메일이 지정가격
  메일보다 먼저 한 번만 발송되고 종목명·현재가·기간고점 대비 위치가 선택
  결과와 일치한다.
...
- [ ] 사용자 정렬은 `상승세·반복 상승폭·반복 횟수·외국인+기관 수급강도·정량점수` 다섯 열에만 존재하고 오름차순·내림차순·해제 순으로 순환한다.
...
- [ ] 복사된 자동매수 승인문에는 보고서 시각·기간·종목코드·진입 목표가·값 출처·주문가능현금 배정률·미배정률·`selection_basis: QUANTITATIVE_OPPORTUNITY`와 `ENTRY_MANDATE`, `ENTRY_APPROVAL`이 명시된다.
...
- [ ] 매 거래일 14일 정량순위 상위 50개의 순위·가격·AI 종합 코멘트·모델·프롬프트·계산 버전을 동결하며 과거 판정을 덮어쓰지 않는다.
...
- [ ] 6번 자율매매 실적 Pages는 공개 전용 화이트리스트 DTO만 사용하고 계좌번호,
  계좌별칭, 증권사, 주문번호, 승인문, 주문가능현금, 사용자 식별자와 원본 KIS
  응답을 HTML·JSON·JavaScript·소스맵·메타데이터·Git 이력에 포함하지 않는다.
- [ ] 6번 Pages의 최초 투자금·현재 투자금·수익률·정제된 매수/매도 이력은 원장과
  산술적으로 일치하고, 공개 필드 검사에 실패하면 이전 정상 배포본을 보존한다.
...
- [ ] 거래일이 일치하는 사용자 우선 종목은 안전·추천·신선도 게이트를 통과한
  경우에만 일반 후보보다 먼저 선정되고, 만료일·중복·잘못된 종목코드는 주문에
  영향을 주지 않는다.
- [ ] `REQUIRE_INCLUDE`는 명시된 종목의 정량 순위만 우회하며 분봉 `READY`, 시장
  재개 잠금, 계좌 대조와 진입 엔진을 우회하지 않고 감사 로그에 정책을 남긴다.
- [ ] 운영 상태 질의는 heartbeat뿐 아니라 보고서·오버레이 신선도, 시장 잠금,
  후보 자격, 차단 이벤트와 메일 경로까지 점검한다.
...
- 51~200위에는 AI 종합 코멘트를 추정해 채우지 않고 `AI 심층검토 대상 외`를 표시한다.
...
- [x] 1~7번 시스템을 번호·상태·현재 작업·최근 성공·다음 실행·문제로 표시한다.
- [x] 별도 바로가기 열 없이 3·4·5·6·7번 시스템 이름에서 공개 대시보드로 연결한다.
- [x] 최근 체결 표의 매수 행 전체는 붉은색, 매도 행 전체는 푸른색 일반체로 표시한다.
- [x] 매도수익률은 별도 열 없이 체결가 뒤에 괄호로 표시하고, SQL 주문 세대의 평균매입가와 실제 체결가로 계산한 비용 전 값임을 표시한다.
- [x] 공개 실적은 5천만원 단일 기준의 순자산·누적손익·누적수익률을 사용한다.
- [x] 계좌번호·증권사·주문번호·승인문·SMTP·API 비밀정보 공개를 차단한다.
- [x] UTF-8 한글 렌더링과 민감 필드 거부 자동 테스트를 유지한다.
- [x] 공개 스냅샷은 최소 15분 주기이며 장외·주말 불필요 갱신을 생략한다.
- [x] 시장 센싱의 과거 중복 예약 작업을 제거하고 통합 런타임으로 단일화한다.
- [x] SQL 누적 체결 원장 멱등성 및 재시작 복구 테스트를 유지한다.
# 2026-08-06 실계좌 승격 체크리스트

- [x] 실계좌 자격증명과 설정 환경 일치
- [x] 실계좌 전용 DB 생성 및 Alembic `0005_fill_ledger`
- [x] 실계좌 전용 토큰 캐시·명령함·알림 outbox 분리
- [x] KIS 실전 주문·매도·취소 TR ID 공식 샘플 대조
- [x] `-3%/-5%` 방어와 `-7%` 불변 시장가 손절 정책 실전 승인본 분리
- [x] 시작 시 KIS 잔고·내부 포지션 대조 PASS
- [x] 시작 시 실계좌 보유 0, 미체결 0 확인
- [x] 유효한 90일 실전 자율 캠페인 생성
- [x] 모의 런타임 종료 및 모의 마감 예약 작업 제거
- [x] 장외시간 신규매수 차단(`OUTSIDE_REGULAR_SESSION`) 감사 확인
- [ ] 다음 정규장 첫 지정가·매수·매도 이메일과 KIS 체결 대조
# 2026-08-06 실계좌 손절 세분화·익절 `+3%` 핫 승격 증거

- 변경 정책: `exit-defense-profit-live-v2-recovery-gated-arm-3`
- 불변 확인: `-7%` 도달 시 회복 신호·관찰시간과 무관하게 전량 시장가 손절
- `-5%` 확인: 회복 점수·매도압력·약세·시장·데이터 조건을 모두 통과한 경우만
  최대 15초 관찰하며, 시간 만료와 `RISK_OFF`에서는 전량매도
- 익절 확인: 최고수익 `+3/+5/+8%` 구간별 반납폭 `1.0/1.5/2.0%p`와 약세 조건 적용
- 자동 검증: 위험 엔진·런타임 핵심 22개 통과, 전체 239개 통과, `git diff --check` 통과
- 배포 전 상태: 실계좌 보유 3종목, 미체결 주문 0건
- 배포 후 상태: `RUNNING`, 보유 `010060/322000/475150` 복구, 미체결 주문 0건,
  정책 승인·실계좌 주문 게이트·DB 스키마·SMTP 점검 통과
- 롤백 기준: 기대하지 않은 즉시청산, `-7%` 지연, KIS·DB 불일치, 반복 주문 또는
  정책 파싱 실패 발생 시 이전 `exit-defense-profit-live-v1-trading-minutes`로 복귀
