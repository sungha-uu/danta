...
- [ ] 하단+10% 목표 도달 횟수는 각 7·14·21일 가격창에서 독립 계산하고 모든 기간에 동일한 고정 예시값을 재사용하지 않는다.
- [ ] 박스 하단 접촉 뒤 이후 1분봉 고가가 `박스 하단 × 1.10` 이상일 때만 목표 도달 1회로 계산한다.
- [ ] 한 1분봉 또는 60분봉이 하단과 +10% 목표가를 모두 접촉했지만 선후관계가 불명확하면 목표 도달을 가산하지 않는다.
- [ ] 기간 말 미도달 사건은 진행 중으로 보존하고 승인된 관찰기한 없이 실패로 임의 분류하지 않는다.
- [ ] 미니차트 클릭 팝업의 거래일·시간·OHLCV가 보고서 DTO와 일치하며 7·14·21일 워밍업 상태를 준수한다.
