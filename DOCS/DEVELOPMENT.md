...
- 7·14·21일 전역 전환: 동일 공식 후보 200개의 기간별 순위·AI 코멘트·수익률·박스·차트·거래대금·수급 동기화
...
### Phase 1: 일별 데이터와 적격 후보

- KOSPI 종목·거래대금·수급·공시와 정규장 1분봉 원본
- 1분봉 품질·수정주가와 재현 가능한 60분봉, 7·14·21일 박스·일중 변동성·다중 목표 상승 탄력성·유동성·붕괴
- 미래 데이터 누수 방지와 재현성

### Phase 2: 정량 후보 200개와 상위 50개 AI 종합 코멘트 Pages

- 정량 상위 50개의 AI 종합 코멘트와 모델·프롬프트 버전
...
- 정량 순위 구간별 성과와 AI 종합 코멘트 주제별 후향 성과 비교
- 적응형 익절 대 고정 익절 비교
...
- 대시보드 `--demo`와 `--input`은 동시에 사용할 수 없으며, 입력 JSON은 200개 후보의 기간별 정량 지표를 가져야 한다. AI 종합 코멘트는 상위 50개에만 존재할 수 있다.
...
- `intraday-report`는 `kospi-market-cap-top200-v1`을 적용하고 최근 21거래일 KIS 1분봉을 종목·거래일 파일로 원자 저장한다. 재실행 시 정규장 커버리지 검증을 통과한 파일을 건너뛰고 미완료 구간부터 재개하며, 완료 후 200개 전체를 공식 주문 후보로 생성하고 정량 상위 50개를 AI 심층검토한다.
...
6. 뉴스·토론·재무 컨텍스트 수집과 공식 후보 전체 7·14·21일 에이전트 검토
7. 사용자 선택 1~3개 집중 감시
