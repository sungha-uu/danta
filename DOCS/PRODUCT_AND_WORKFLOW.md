...
후보 우선순위는 `1차 적격성 → 6% 이상 비중복 상승 횟수 → 평균 반복 상승폭 → 평균 6% 도달시간` 순서다. 외국인·기관 수급과 뉴스·공시는 정량 상위 50개의 AI 심층 검토에 사용하며 1차 정량 순위를 뒤섞지 않는다.
...
```mermaid
flowchart TD
    A["장 마감 데이터 수집"] --> B["파이썬 공개 정량 순위 200개"]
    B --> C["AI가 상위 50개 기간별 등급·근거 심층검토"]
...
2. 결정적 정량 엔진이 KOSPI 시총 상위 200개의 7·14·21일 1분봉에서 6% 이상 비중복 상승 구간을 계산한다. 14일의 반복 횟수·평균 상승폭·평균 도달시간 순으로 1~200 고정 순번을 만들고, 7·21일 탭은 같은 순서에서 기간별 값만 바꾼다.
3. AI는 정량 상위 50개만 수급·뉴스·공시·토론·이벤트 위험을 심층 검토한다. 51~200위는 로컬 정량 데이터만 제공해 AI 사용량을 통제한다.
4. 정량 후보 200개 전체와 상위 50개의 AI 등급·코멘트·근거·위험을 민감정보 없이 정적 페이지에 배포한다.
...
- 사용자는 검색 가능한 단일 `정량 순위 200개` 종합표에서 1~3개를 고르고, 표 위 상단 선택창에서 현재 기간의 박스 하단가가 자동 입력된 종목별 진입 목표가를 수정할 수 있다. 상단 선택창은 기본 숨김이며 최상단 날짜 옆 단일 토글로 표시·숨김을 전환한다.
