...
1. **시장 센싱 시스템**: KOSPI 지수, 시장 폭, 개인·외국인·기관 및 기관 세부, 연기금 등, 프로그램 수급과 데이터 품질을 감시한다. `RISK_OFF/PANIC` 또는 공급자 불완전 시 신규매수를 잠그며 시장이 회복돼도 사용자 확인 전에는 자동 재개하지 않는다.
2. **자율매매 시스템**: 후보 선정, 실시간 오버레이, 진입, 주문·체결, 보유 감시, 손절·익절, 복구를 담당한다. 1번의 신규진입 잠금을 우회할 수 없고 열린 포지션 보호는 계속한다.
3. **대시보드 시스템**: `3-1 단기 후보`, `3-2 KOSPI 시장·자금 흐름`, `3-3 계좌·자율매매 실적`으로 나눈다. 공개 Pages에는 계좌·주문·비밀정보를 싣지 않으므로 3-3은 비공개 또는 인증된 로컬/서버 화면으로 구현한다.
4. **안전·감사 기반**: 주문 멱등성, KIS 잔고·주문·체결 대조, 재시작 복구, 알림 Outbox, 품질 보증과 전략 버전 승격을 세 시스템 공통 기반으로 둔다.

`1 → 2`는 실시간 위험 게이트이고, `1·2 → 3`은 읽기 전용 관측 경로다. 대시보드·메일 장애가 1번 센싱이나 2번 보호매도를 멈추게 해서는 안 된다.
