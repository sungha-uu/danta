...
> 튼튼하고 거래 가능한 KOSPI 종목을 먼저 걸러낸 뒤, 그 안에서 현재 유효한 활성 박스를 찾고, 하단권에 있으면서 외국인·기관 자금이 유입되는 숨은 반등 후보를 발굴한다.

여기서 “튼튼함”은 수익 보장이나 장기 우량주 판정이 아니라 시가총액·가격·거래대금·거래상태·주문 안전성으로 1차 필터를 통과했다는 의미다. “숨은 후보”는 이미 급등한 종목이 아니라 활성 박스의 하단권에 있고 구조 무효화선을 지키며 상단 재도달 표본과 수급 확인이 있는 종목이다.

후보 우선순위는 `1차 적격성 → 활성 박스 유효성 → 활성 하단 위치 → 재도달 품질 → 외국인·기관 수급 → 뉴스·공시 위험` 순서다. 많이 하락했다는 이유만으로 우선순위를 높이지 않는다.
...
- AI·정량 엔진은 승인된 종목·목표가·사용자 배정률·알고리즘·박스 유효조건 안에서 진입 시점을 결정할 수 있다.
...
```mermaid
flowchart TD
    A["장 마감 데이터 수집"] --> B["파이썬 공개 검토 후보 50개"]
    B --> C["AI가 후보 50개 전체 기간별 등급·근거 검토"]
    C --> D["GitHub Pages 대시보드 배포"]
    D --> E["후보 이메일·대시보드"]
    E --> F["사용자 대시보드 코멘트"]
...
2. 결정적 정량 엔진이 사전필터 통과군에서 7일 정량 상위 50개 비교군을 만들고, +10% 자격 게이트 통과 여부와 탈락 사유를 함께 기록한다.
3. AI는 30개 밖의 종목을 추가하지 않고 지속성·이벤트 위험을 검토한다.
4. 후보 50개 전체의 7·14·21일별 AI 등급·코멘트·근거·위험을 민감정보 없이 정적 페이지에 배포한다. 비추천 종목도 숨기지 않아 사용자가 필터의 거짓음성과 AI 판단 오류를 검토할 수 있게 한다.
...
- 사용자는 단일 `검토 후보 50개` 종합표에서 1~3개를 고르고, 표 위 상단 선택창에서 현재 기간의 박스 하단가가 자동 입력된 종목별 진입 목표가를 수정할 수 있다. 상단 선택창은 기본 숨김이며 최상단 날짜 옆 단일 토글로 표시·숨김을 전환한다.
...
- 위임은 전량 체결 또는 박스 무효화까지 유효하다. 거래일 종료 등 브로커 주문 수명이 끝나면 로컬 엔진이 박스를 재검증한 뒤 필요한 주문만 재제출한다.
...
사용자는 대시보드에서 최대 3개 종목과 목표가를 정한 뒤 `ENTRY_MANDATE` 승인문을 Android Codex에 붙여넣는다. 에이전트가 스키마·현재 잠긴 계좌 모드·데이터 신선도·박스 유효성을 검증해 로컬 제어 API에 기록하면 사용자가 매수 버튼을 다시 누르지 않아도 로컬 진입 엔진이 목표가 도달 시 자동 주문한다. 선택만 있고 승인문 전달이 없으면 주문하지 않는다.
...
- 미체결분은 체결까지 유지하되 박스 경계·시장 위험·데이터 품질이 무효화되면 취소한다.
...
AI 사용량 부족으로 후보 30개 전체 검토가 완료되지 않으면 미검토 종목에 `AI_REVIEW_QUOTA_DEGRADED`를 표시하고 등급을 추정해 채우지 않는다. AI 정책이 필요한 자동 진입은 새 AI 판단 없이 실행하지 않으며, 기존 포지션의 손절·익절은 로컬 엔진이 계속 수행한다.
