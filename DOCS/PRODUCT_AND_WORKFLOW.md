...
- 데이터: 16:00 전일 종가 기준 분석이 직전 거래일보다 오래되면 자동 진입을
  차단한다. 배치 생성 직전 KIS 현재가를 다시 조회하고, AI 등급 우선·실시간
  박스 위치가 낮은 순·14일 순위 순으로 최대 3개를 선택한다.
- 동시 보유: 계좌 전체 기준 최대 3종목, 미보유 종목만 신규 진입
- 진입: 배치 생성 직전에 다시 조회한 KIS 현재가를 최대 매수가로 사용하되 기존
  하락 안정·매수세 회복 엔진이 실제 지정가를 확정한다.
- 재진입: 동일 보고서·동일 거래일에는 한 번만 자동 배치를 생성한다.
...
- 성과 기준: 최초 원금 `50,000,000원` 대비 누적 손익과 누적수익률만 사용한다.
