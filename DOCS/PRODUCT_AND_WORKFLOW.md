...
기존 장기보유 계좌와 분리된 신규 1,000만 원 단기매매 계좌에서 반복 가능한 실현수익을 만드는 자동화 시스템을 구축한다. 변동성이 크고 유동성이 충분하며 넓은 박스 안에서 반복 왕복하는 KOSPI 종목을 찾고, 사용자의 매수 판단을 보조하며, 승인된 매수 이후 청산을 자동화한다.
...
1. KOSPI 일봉·거래대금·수급·공시를 수집하고 품질을 검사한다.
2. 결정적 정량 엔진이 후보 30개를 만든다.
3. AI는 30개 밖의 종목을 추가하지 않고 지속성·이벤트 위험을 검토한다.
4. 후보 30개 전체의 AI 등급·코멘트·근거·위험을 민감정보 없이 정적 페이지에 배포한다.
...
- 사용자는 단일 후보 30 종합표의 체크박스로 1~3개를 고르고, 표 위 상단 선택창에서 현재 기간의 박스 하단가가 자동 입력된 종목별 진입 목표가를 수정할 수 있다. 상단 선택창은 기본 숨김이며 최상단 날짜 옆 단일 토글로 표시·숨김을 전환한다.
