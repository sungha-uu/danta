...
기존 장기보유 계좌와 분리된 KIS 실계좌 단기매매 계좌에서 반복 가능한 실현수익을 만드는 자동화 시스템을 구축한다. 실계좌 자율매매의 최초 원금과 모든 누적 성과 기준은 `5,000,000원` 하나만 사용한다. 과거 모의운용 성과는 활성 실계좌 수익률과 합산하지 않고 감사 이력으로만 보관한다.
...
후보 우선순위는 `6% 이상 비중복 상승 횟수 → 평균 반복 상승폭 → 평균 6% 도달시간` 순서다. 시가총액 상위 200개는 모두 주문 선택 가능하다. 외국인·기관 수급과 뉴스·공시는 정량 상위 50개의 AI 심층 검토에 사용하며 1차 정량 순위를 뒤섞지 않는다.
...
앞으로 기능·장애·성과를 다음 번호로 관리한다.

1. **통합 운영·안전 코어**: 전체 프로세스 기동·복구, 종목별 워커와 주문 우선순위, 자금 예약, 시장 위험 게이트, KIS 대조, 감사 로그, 알림 Outbox, 예약 작업과 시스템 헬스체크를 총괄한다.
2. **자율매매 시스템**: 후보 선정, 실시간 오버레이, 진입, 주문·체결, 보유 감시, 손절·익절과 복구를 담당한다. 1번의 신규진입 잠금을 우회할 수 없고 열린 포지션 보호는 계속한다.
3. **시장 센싱 시스템**: KOSPI 지수, 시장 폭, 개인·외국인·기관 및 기관 세부, 연기금 등, 프로그램 수급과 데이터 품질을 감시한다. `RISK_OFF/PANIC` 또는 공급자 불완전 시 1번 코어에 신규매수 잠금을 요청하며 시장이 회복돼도 사용자 확인 전에는 자동 재개하지 않는다.
4. **단기 후보 대시보드**: 시총 상위 200개 정량 결과와 상위 50개 AI 심층검토, 7·14·21일 데이터와 사용자 승인문 생성을 제공한다.
5. **KOSPI 시장·자금 흐름 대시보드**: 3번 센싱의 공개 가능 시장 상태와 수급을 지연된 정적 화면으로 제공한다.
6. **자율매매 실적 대시보드**: 사용자가 공개를 승인한 최초 투자금·현재 투자금·수익률·정제된 매수/매도 이력만 GitHub Pages에 제공한다. 계좌번호·증권사·주문번호·승인문·주문가능현금·원본 브로커 응답은 포함하지 않는다.
7. **AI 종목 추천 및 재무제표 대시보드**: 별도 재무분석 프로젝트가 생성하는 KOSPI 재무제표 분석·AI 추천 현황을 연결한다. Danta 주문 엔진의 입력 원본은 아니며 통합 운영 화면에서는 공개 페이지 연결과 갱신 상태만 관제한다.

1번 코어의 통합 운영 대시보드는 1~7번의 상태·마지막 정상 시각·다음 예약 시각·현재 작업·경고·최근 조치를 제공한다. 별도 바로가기 열은 두지 않고 공개 화면이 있는 3·4·5·6·7번 시스템 이름 자체를 링크로 표시한다. 6번 실적 화면의 최근 체결 표는 매수 행 전체를 붉은색, 매도 행 전체를 푸른색 일반체로 구분한다. 매도수익률은 별도 열을 만들지 않고 체결가 바로 뒤에 괄호로 표시하며 수익은 붉은색 `(+n.nn%)`, 손실은 푸른색 `(-n.nn%)`으로 표시한다. 값은 같은 종목·포지션 세대의 SQL 평균매입가와 실제 매도 체결가로 계산한 비용 전 수익률이며, 수수료·세금까지 반영한 순수익률로 오인하지 않도록 화면에 산식 기준을 명시한다. 상태 산출은 Codex가 수동으로 적는 방식이 아니라 로컬 Python 헬스체크의 기계 판정이어야 한다. 사용자는 통합 화면에서 문제를 확인한 뒤 Android Codex로 수정을 요청하고, 에이전트는 진단·수정·검증·트러블슈팅 기록 후 상태를 갱신한다.

`3 → 1 → 2`는 실시간 위험 게이트이고, `1·2·3 → 4·5·6`과 독립 프로젝트 `7`은 읽기 전용 관측 경로다. 대시보드·메일 장애가 시장 센싱이나 보호매도를 멈추게 해서는 안 된다.
...
4. 시가총액 상위 200개 전부를 공식 주문 후보로 제공한다. 현재 하단 위치와 과거 +10% 도달 이력은 사용자의 판단 및 AI 추천 근거로 제공하지만 선택을 차단하지 않는다.
...
- `자동매수 승인문 복사`가 보고서 시각·분석기간·종목·진입 목표가·주문가능현금 배정률·정량 기회 선정 근거와 내부 실행정책을 `ENTRY_MANDATE`로 구조화하고, 사용자가 이를 Android Codex 앱에 붙여넣으면 현재 잠긴 계좌 모드에서 자동 진입 승인이 성립한다. 과거 승인문 호환을 위해 `ai_grade`를 읽을 수는 있지만 새 승인문에는 생성하지 않는다.
...
## 6. 사용자 승인형 진입 위임과 자동 타이밍

사용자는 대시보드에서 최대 3개 종목과 목표가를 정한 뒤 `ENTRY_MANDATE` 승인문을 Android Codex에 붙여넣는다. 에이전트가 스키마·현재 잠긴 계좌 모드·데이터 신선도·박스 유효성을 검증해 로컬 제어 API에 기록하면 자동 진입 위임이 성립한다. 목표가는 즉시 체결가가 아니라 절대 초과할 수 없는 최대 매수가이며, 가격 도달 후에도 매도 압력·하락 안정·매수세 회복을 로컬 엔진이 확인한 뒤 같은 가격 이하의 지정가 주문만 제출한다. 기본 경로에서는 선택만 있고 승인문 전달이 없으면 주문하지 않는다. 세부 기준은 [`ENTRY_AND_BUY.md`](ENTRY_AND_BUY.md)를 따른다.

### 90일 KIS 실계좌 자율매매 캠페인

사용자가 명시적으로 `AUTONOMOUS_TRADING_CAMPAIGN`을 승인하면 승인 시점부터 최대
90일 동안 현재 KIS 실계좌에서 종목별 승인문 없이 추천 후보의 매수·매도를 반복한다.
KIS 주문가능금액, 현금주문, 체결조회, 정정·취소, 잔고조회 API를 실전 서버에 호출한다.

- 대상: 시가총액 상위 200개 중 14일 분봉 구조와 장중 데이터가 준비된 종목
- 데이터: 16:00 완결 보고서가 직전 거래일보다 오래되면 자동 진입을 차단한다.
  KRX 정규장에는 200종목의 1분봉을 30분마다 증분 재평가한다. 신선한 14일 장중
  반복 상승 정량 순위·기간 고점 대비 할인 폭·장중 수급을 함께 사용해 최대 3개를 선택한다.
- 동시 보유: 계좌 전체 기준 최대 3종목, 미보유 종목만 신규 진입
- 진입: 배치 생성 직전에 다시 조회한 KIS 현재가를 최대 매수가로 사용하되 기존
  하락 안정·매수세 회복 엔진이 실제 지정가를 확정한다.
- 재진입: 같은 장중 revision은 한 번만 처리한다. 기존 배치가 전량 청산되고 계좌가
  평탄할 때만 다음 revision으로 재선정하며 하루 최대 3배치다.
- 중단: 만료, 로컬 kill switch, 시장 `RISK_OFF/PANIC`, 일일 신규 배치 한도,
  데이터 지연, 계좌·주문 불일치 때 신규매수를 즉시 차단한다.
- 종료 보호: 캠페인 중단 후에도 보유 포지션의 손절·익절·잔고대조는 계속한다.
- 격리: 전용 실계좌 DB·토큰·명령함을 사용하며 과거 모의 상태를 읽지 않는다.
- 정책 변경: 오프라인 검증과 사용자 승인 없이 실전 정책 버전을 자동 변경하지 않는다.
- 보고: 자율 캠페인 거래일마다 15:35에 KIS 체결·잔고를 권위 원본으로
  대조해 제목 `# 자율매매 현황`의 일일 마감 메일을 보낸다.
  매수·매도 체결, 현재 보유종목, 평가손익과 계좌 수익률을 포함하며
  일일 보고 실패가 보호매도 경로를 지연시키지 않는다.
- 성과 기준: 최초 원금 `5,000,000원` 대비 누적 손익과 누적수익률만 사용한다.
  첫 자율 캠페인 승인 시점의 계좌 평가액을 별도 기준자산으로 사용하지 않는다.

자율 배치는 `직전 완료 거래일 데이터 갱신 → 상위 50개 에이전트 심층검토 →
추천 후보 장중 오버레이 → 최대 3개 종목 선정 메일 → 지정가격 메일 →
진입 감시·자동매수 → 자동매도` 순서로 실행한다. 일일 파이프라인과 자율매매는
`data/candidate_intraday_ai_report.json`을 단일 기준 보고서로 공유하며,
공개 Pages를 주문 입력으로 다시 읽지 않는다.

시장 위험·영속 재개 잠금·기존 활성 명령은 후보 선정과 감시 지정가 계산을 막지
않는다. 최신 후보 묶음을 비공개 대기 스냅샷으로 계속 갱신하고 종목 구성이 바뀔 때
선정·지정가 메일을 보낸다. 다만 실제 신규 주문은 모든 실행 게이트가 통과할 때까지
대기하며 거래일당 한 번 `자율매매 신규진입 보류.` 메일로 차단 사유를 알린다.
...
- 동일 종목 미보유·활성 주문 없음. 손절 당일이라도 최신 가격·수급·반등 자격을
  새로 통과하면 재진입 가능
