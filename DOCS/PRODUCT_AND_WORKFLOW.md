...
flowchart TD
    ...
    B --> C["+10% 자격 게이트 공식 후보 최대 30개"]
    C --> D["AI가 공식 후보 전체 기간별 등급·근거 심층검토"]
    D --> PAGES["GitHub Pages 대시보드 배포"]
    PAGES --> E["후보 이메일·대시보드"]
    E --> F["사용자 대시보드 코멘트"]
...
3. 14일 기준 현재 박스 하단 35% 이내이고 하단 접촉 후 3거래일 이내 실제 +10% 도달 이력이 있는 종목만 공식 후보로 선택하며 최대 30개로 제한한다.
4. AI는 공식 후보 전체의 수급·뉴스·공시·토론·재무 위험을 심층 검토하고 민감정보 없이 정적 페이지에 배포한다.
