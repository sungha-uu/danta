...
```mermaid
flowchart TD
    A["장 마감 데이터 수집"] --> B["파이썬 정량 후보 30개"]
    B --> C["AI가 후보 30개 전체 등급·근거 검토"]
...
2. 결정적 정량 엔진이 후보 30개를 만든다.
3. AI는 30개 밖의 종목을 추가하지 않고 지속성·이벤트 위험을 검토한다.
4. 후보 30개 전체의 AI 등급·코멘트·근거·위험과 비교용 31~50위 확장 관찰군을 민감정보 없이 정적 페이지에 배포한다.
...
- 사용자는 단일 `공식 후보 30 + 확장 관찰 20` 종합표에서 공식 후보의 체크박스로만 1~3개를 고르고, 표 위 상단 선택창에서 현재 기간의 박스 하단가가 자동 입력된 종목별 진입 목표가를 수정할 수 있다. 31~50위 확장 관찰군은 비교 전용으로 선택할 수 없다. 상단 선택창은 기본 숨김이며 최상단 날짜 옆 단일 토글로 표시·숨김을 전환한다.
