...
기존 장기보유 계좌와 분리된 단기매매 계좌에서 반복 가능한 실현수익을 만드는 자동화 시스템을 구축한다. KIS 모의계좌 자율매매의 최초 원금과 모든 누적 성과 기준은 `50,000,000원` 하나만 사용한다. 사용자가 종목별 승인을 하던 초기 모의운용도 같은 자율매매 검증 이력에 포함하며, 별도의 “캠페인 시작 시점 기준자산”을 만들지 않는다.
...
### 90일 KIS 모의계좌 자율 검증 캠페인

사용자가 명시적으로 `PAPER_AUTONOMOUS_CAMPAIGN`을 승인하면 승인 시점부터 최대
90일 동안 KIS 모의계좌에서만 종목별 승인문 없이 추천 후보의 매수·매도를 반복한다.
이는 그림자 체결이 아니라 KIS의 주문가능금액, 현금주문, 체결조회, 정정·취소,
잔고조회 API를 실제 모의 서버에 호출하는 통합 검증이다.
...
- 격리: `environment=prod` 또는 실전 KIS 자격증명에서는 어떤 설정으로도 거부한다.
- 승격 금지: 모의 성과가 좋아도 실계좌 정책으로 자동 승격하지 않는다.
- 보고: 자율 캠페인 거래일마다 15:35에 KIS 체결·잔고를 권위 원본으로
  대조해 제목 `# 자율 모의투자 현황`의 일일 마감 메일을 보낸다.
