...
1번 코어의 통합 운영 대시보드는 1~6번의 상태·마지막 정상 시각·다음 예약 시각·현재 작업·경고·최근 조치를 제공한다. 별도 바로가기 열은 두지 않고 공개 화면이 있는 2·4·5·6번 시스템 이름 자체를 링크로 표시한다. 6번 실적 화면의 최근 체결 표는 `구분·수량·체결가` 실제 값에 한해 매수를 붉은색, 매도를 푸른색으로 구분한다. 상태 산출은 Codex가 수동으로 적는 방식이 아니라 로컬 Python 헬스체크의 기계 판정이어야 한다. 사용자는 통합 화면에서 문제를 확인한 뒤 Android Codex로 수정을 요청하고, 에이전트는 진단·수정·검증·트러블슈팅 기록 후 상태를 갱신한다.
