...
후보 우선순위는 `1차 적격성 → 하단 접촉 후 실제 +10% 도달 이력 → 선택 기간 전체의 현재 하단 위치 → +10% 목표가가 현재가보다 높음 → 외국인·기관 수급 → 뉴스·공시 위험` 순서다. 활성 상단 재도달과 활성 위치는 추천 자격이나 순위를 대체할 수 없으며, 많이 하락했다는 이유만으로 우선순위를 높이지 않는다.
...
2. 결정적 정량 엔진이 KOSPI 시총 상위 200개의 7·14·21일 차트 모양·진폭·현재 위치를 계산해 복합 정량 상위 50개 비교군을 만들고, 좋은 눌림·구조적 붕괴와 +10% 이력을 함께 기록한다.
3. AI는 정량 상위 50개 밖의 종목을 추가하지 않고 수급·뉴스·공시·지속성·이벤트 위험을 심층 검토한다.
...
AI 사용량 부족으로 후보 50개 전체 검토가 완료되지 않으면 미검토 종목에 `AI_REVIEW_QUOTA_DEGRADED`를 표시하고 등급을 추정해 채우지 않는다. AI 정책이 필요한 자동 진입은 새 AI 판단 없이 실행하지 않으며, 기존 포지션의 손절·익절은 로컬 엔진이 계속 수행한다.
