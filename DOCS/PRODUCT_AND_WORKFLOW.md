...
```mermaid
flowchart TD
    A["장 마감 데이터 수집"] --> B["파이썬 적격 후보 최대 30개"]
    B --> C["AI가 적격 후보 전체 등급·근거 검토"]
...
2. 결정적 정량 엔진이 +10% 자격 게이트를 통과한 후보를 최대 30개 만들며 숫자를 맞추기 위해 부적격 종목을 넣지 않는다.
3. AI는 30개 밖의 종목을 추가하지 않고 지속성·이벤트 위험을 검토한다.
4. 적격 후보 전체의 AI 등급·코멘트·근거·위험을 민감정보 없이 정적 페이지에 배포한다. 정량 상위 50개는 필터 성능 평가용 비공개 감사 풀로만 보존한다.
...
- 사용자는 단일 `적격 후보 N개` 종합표에서 1~3개를 고르고, 표 위 상단 선택창에서 현재 기간의 박스 하단가가 자동 입력된 종목별 진입 목표가를 수정할 수 있다. 상단 선택창은 기본 숨김이며 최상단 날짜 옆 단일 토글로 표시·숨김을 전환한다.
