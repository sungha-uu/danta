...
기존 장기보유 계좌와 분리된 신규 1,000만 원 단기매매 계좌에서 반복 가능한 실현수익을 만드는 자동화 시스템을 구축한다. 변동성이 크고 유동성이 충분하며 현재가가 최근 구조의 하단에 가깝고 하단 대비 +10% 상승 탄력성이 확인된 KOSPI 종목을 찾고, 사용자의 매수 판단을 보조하며, 승인된 매수 이후 청산을 자동화한다.
