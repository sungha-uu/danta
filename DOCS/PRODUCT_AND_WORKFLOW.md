...
2. **시장 센싱 시스템**: KOSPI 지수, 시장 폭, 개인·외국인·기관 및 기관 세부, 연기금 등, 프로그램 수급과 데이터 품질을 감시한다. `RISK_OFF/PANIC` 또는 공급자 불완전 시 1번 코어에 신규매수 잠금을 요청하며 시장이 회복돼도 사용자 확인 전에는 자동 재개하지 않는다.
3. **자율매매 시스템**: 후보 선정, 실시간 오버레이, 진입, 주문·체결, 보유 감시, 손절·익절과 복구를 담당한다. 1번의 신규진입 잠금을 우회할 수 없고 열린 포지션 보호는 계속한다.
4. **단기 후보 대시보드**: 시총 상위 200개 정량 결과와 상위 50개 AI 심층검토, 7·14·21일 데이터와 사용자 승인문 생성을 제공한다.
5. **KOSPI 시장·자금 흐름 대시보드**: 2번 센싱의 공개 가능 시장 상태와 수급을 지연된 정적 화면으로 제공한다.
...
1번 코어의 통합 운영 대시보드는 1~6번의 상태·마지막 정상 시각·다음 예약 시각·현재 작업·경고·최근 조치를 제공한다. 별도 바로가기 열은 두지 않고 공개 화면이 있는 2·4·5·6번 시스템 이름 자체를 링크로 표시한다. 6번 실적 화면의 최근 체결 표는 매수 행 전체를 붉은색, 매도 행 전체를 푸른색 일반체로 구분한다. 매도에는 같은 종목·포지션 세대의 SQL 평균매입가와 실제 매도 체결가로 계산한 비용 전 `매도수익률`을 함께 표시하고 매수 행에는 `-`를 표시한다. 수수료·세금까지 반영한 순수익률로 오인하지 않도록 산식 기준을 화면에 명시한다. 상태 산출은 Codex가 수동으로 적는 방식이 아니라 로컬 Python 헬스체크의 기계 판정이어야 한다. 사용자는 통합 화면에서 문제를 확인한 뒤 Android Codex로 수정을 요청하고, 에이전트는 진단·수정·검증·트러블슈팅 기록 후 상태를 갱신한다.

`2 → 1 → 3`은 실시간 위험 게이트이고, `1·2·3 → 4·5·6`은 읽기 전용 관측 경로다. 대시보드·메일 장애가 시장 센싱이나 보호매도를 멈추게 해서는 안 된다.
