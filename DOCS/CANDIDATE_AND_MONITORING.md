...
`+10% 상승 탄력성`은 7·14·21일 창마다 독립 계산한다. 박스 하단은 60분봉 저가의 10분위, 목표가는 `박스 하단 × 1.10`이다. 원본 1분봉 저가가 하단 이하로 내려간 첫 시점을 한 진입 관찰 사건으로 열고, 그 이후 1분봉 고가가 목표가 이상에 도달하면 성공 1회로 닫는다. 성공 뒤 다시 하단에 접촉해야 다음 사건을 연다. 동일 1분봉에서 하단과 목표가를 모두 기록한 경우 선후관계를 알 수 없어 성공으로 세지 않는다. 기간 끝까지 목표에 닿지 않은 열린 사건은 `진행 중`으로 표시하며 실패율 계산에 임의로 넣지 않는다.

```text
target_price_10pct = box_low × 1.10
lower_contact_count = 성공 사건 + 기간 말 진행 중 사건
target_reach_count = 하단 접촉 뒤 목표가에 도달해 닫힌 사건
```

지속 가능성이 높은 구조:

- 상단·하단 접촉이 양쪽에 고르게 존재
- 하단 접촉 뒤 +10% 목표 도달 횟수와 최신성이 충분
- 목표 도달 사건이 한 시점에만 몰리지 않음
...
| 탄력성 | 하단 접촉, 하단+10% 목표 도달 횟수·소요시간·최신성 |
...
```text
quant_score = target10_elasticity + amplitude + liquidity + lower_position + box_inclusion
...
- 하단+10% 도달이 반복 가능한 반등인지 일회성 급등인지
...
- 종목명·코드·업종·현재가와 선택 기간 박스 하단·상단·진폭·현재 위치 또는 분봉 수집 상태
...
- 미니차트를 누르면 선택 기간의 60분봉 확대 흐름과 각 봉의 거래일·시간·시가·고가·저가·종가·거래량을 정적 페이지 내부 팝업으로 표시한다. 브라우저에서 외부 시세 API를 호출하지 않는다.
...
### 10.1 활성 목표 버전: `box-elasticity-v3-60m-target10`
...
- 상승 탄력성: 원본 1분봉에서 하단 접촉 뒤 `하단 × 1.10` 목표가 도달 횟수
- 초기 상대 정량 가중치: +10% 도달 35%, 진폭 20%, 유동성 20%, 하단 근접도 15%, 박스 포함률 10%
