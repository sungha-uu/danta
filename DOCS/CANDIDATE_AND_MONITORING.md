...
| AI 심층검토 | 14일 정량 순위 상위 50개의 수급·뉴스·공시·토론·재무 위험 |
...
AI는 14일 정량 순위 상위 50개를 심층검토하며 주문 선택 권한이나 원시 숫자를 바꿀 수 없다. 다음을 검토한다.
...
AI 출력에는 정량 상위 50개에 대한 `적극 추천/추천/비추천/적극 비추천`, 코멘트, 근거, 위험, 무효화 조건, 데이터 시각, 모델 ID, 프롬프트 버전을 포함한다. 51~200위는 등급을 추정하지 않고 `AI 심층검토 대상 외`로 표시한다. AI 검토가 실패한 종목은 `AI_REVIEW_MISSING`으로 표시한다.
...
GitHub Pages에는 시총 상위 200개를 모두 게시한다. 7·14·21일 탭은 동일한 14일 기준 순번 1~200을 유지하고 각 기간의 실제 분봉 지표·수급·등급·코멘트만 변경한다. 상위 50개에는 AI 심층검토 결과를 표시하고, 200개 모든 행에 선택 체크박스와 주문 승인문 생성 권한을 준다.
...
- 실시간 오버레이는 200개 전수 정량 재평가다. 외부 AI를 30분마다 200개에 다시
  호출하지 않으며, 직전 16:00 보고서에서 AI 심층검토가 완료된 상위 50개 중
  `적극 추천·추천`인 종목만 자동 진입 검토군으로 유지한다.
...
- 해당 거래일 자율 후보 정렬에서 유효한 사용자 우선 종목을 먼저 배치하고 나머지
  슬롯은 기존 `14일 장중 순위 → AI 등급 → 현재 위치` 순서로 채운다. 사용자 지정
  순서가 여러 개면 그 순서를 유지한다.
- 우선 지정은 AI `적극추천·추천`, 14일 상위 50개 심층검토, 분봉 완성도·신선도,
...
- 사용자가 특정 거래일에 종목의 `필수 포함`을 명시하면 해당 종목에 한해 AI 추천
  등급 게이트를 우회할 수 있다. 단, 시총 상위 200개 공식 후보, 14일 분봉·구조
...
후보표의 사용자 정렬은 `상승세`, `반복 상승폭`, `반복 횟수`, `외국인+기관 수급강도`, `정량점수`, `에이전트 점수` 여섯 열에만 제공한다. 각 제목을 누를 때 `오름차순 → 내림차순 → 정렬 해제`로 순환하고, 값이 없는 행은 방향과 무관하게 뒤에 둔다. 활성 정렬 열은 다른 배경색과 방향 표시로 구분한다. 정렬 상태는 7·14·21일 탭 전환에도 유지하되 새 기간의 값으로 다시 정렬한다. `순위`를 누르면 다른 정렬을 모두 해제하고 14일 고정 공식 순위 1위부터 200위까지 복원한다.
