...
- 차트 팝업의 현재가 기준선은 상단·하단선보다 얇은 검은색 점선으로 표시한다. `상단(현재가+10%)`·`하단`·`현재가` 이름은 각 기준선의 오른쪽 끝에 선 색상으로 표시하고, 가까운 선의 문구가 겹치지 않도록 위·아래로 배치한다.
