...
현재 계산 버전은 `intraday-elasticity-v10-top200-good-pullback-v1`이다.

### 3.0.1 좋은 눌림과 구조적 붕괴

- `좋은 하락(GOOD_PULLBACK)`은 현재 위치가 기간 박스 하단 35% 이내이고, 최근 구간의 일중 진폭과 저점 이후 반등폭이 기간 초반 대비 유지되며, 기간 상단이 하단과 함께 가파르게 낮아지지 않은 상태다.
- `나쁜 하락(STRUCTURAL_DECLINE)`은 기간 상단과 하단이 모두 8% 이상 낮아지고 최근 저점 반등폭이 기간 초반의 75% 미만으로 축소된 상태다.
- 점수는 현재 하단 위치 25%, 반복 진폭 20%, 저점 반등력 20%, 하락 건강도 15%, 실제 +10% 도달 이력 10%, 유동성 10%로 구성한다.
- 단일 기간의 우연한 모양을 줄이기 위해 200개 전체를 7·14·21일 각각 계산한 뒤 `7일 50% + 14일 30% + 21일 20%` 복합 점수로 상위 50개를 정한다.
- 위 임계값과 가중치는 `PAPER_CHALLENGER` 연구판이며 성과·거짓양성·거짓음성을 누적 검증한다. 일일 결과만으로 자동 변경하거나 실전판으로 승격하지 않는다.
...
GitHub Pages에는 7·14·21일 복합 정량 상위 50개 비교군을 모두 게시한다. 자격 게이트 통과 여부는 공개 여부를 결정하지 않으며 기간별 AI 등급과 코멘트에 반영한다. 7·14·21일 탭은 동일 종목군을 사용해 기간별 비교 가능성을 유지하고, 각 기간의 실제 60분봉·수급으로 순위·지표·등급을 다시 표시한다. 사용자는 비추천 종목을 포함해 50개 전체에서 누락·과대평가·과소평가를 검토할 수 있다.
...
- 하락률 단독 감점은 사용하지 않는다. 반복 진폭과 반등폭이 유지되는 하단 눌림은 기회로, 상·하단 동반 하락과 반등폭 축소는 구조적 붕괴로 분리한다.
...
공개 검토 후보는 7·14·21일 복합 정량 상위 50개로 고정하며 자격 미달 종목도 숨기지 않는다. 각 종목에는 자격 여부와 사유, 좋은 눌림·구조적 붕괴 판정 근거, 이후 +10% 도달 여부, MAE, 비용 차감 수익을 저장해 거짓양성·거짓음성을 측정한다. 상위 200개 전체군은 내부 연구 데이터로 보존하되 현재의 폭이 넓은 종합표에는 게시하지 않는다.
