...
현재 계산 버전은 `intraday-repeat-rise-v11-top200-fixed14-v1`이다.
...
- 공개 200개와 1~200 순번은 14일 정량 순위로 고정한다. 7·14·21일 탭은 같은 행 순서를 유지하며 해당 기간 지표만 변경한다.
...
현재 위치·하단 방향·좋은 눌림·구조적 붕괴·뉴스·수급은 이 1차 정렬을 바꾸지 않는다. 이 값들은 상위 50개 AI 심층검토와 사용자가 실제 진입 종목을 고르는 단계에서 별도로 해석한다.

## 4. 공개 정량 순위 200개와 AI 심층검토 50개

| 영역 | 특징 |
| --- | --- |
| 반복 상승 | 평균 상승폭, 6% 이상 비중복 횟수, 평균 6% 도달시간 |
| 가격 참고 | 기간 최저가, +10% 목표가, 기간 최고가, 현재 위치, 하단 방향 |
| 적격성 | KOSPI 보통주 시가총액 상위 200개와 주문 가능 상태 |
| AI 심층검토 | 14일 상위 50개의 수급·뉴스·공시·토론 |

```text
sort_key = (-repeat_count, -average_up_swing_pct, average_time_to_6pct, symbol)
```

동일 데이터와 `calculation_version`이면 동일한 200개 비교군과 14일 고정 순위가 나와야 한다. 점수, 특징, 기준시각과 AI 심층검토 대상 여부를 저장하며 미래 데이터를 사용하지 않는다.
...
에이전트 컨텍스트 검토판은 14일 정량 상위 50개의 7·14·21일 반복 상승 구조, 외국인·기관 수급, 뉴스 감성, 토론 참고 신호를 결합한다. 51~200위는 AI 사용량을 통제하기 위해 정량 지표·차트·수급만 표시하고 AI 등급·코멘트·뉴스·토론 요약을 생성하지 않는다. 외부 LLM이 연결되지 않은 단계에서는 `agent-context-review-v1`처럼 로컬·규칙 보조 검토임을 모델 버전에 명시하고 완전한 생성형 AI 검토로 가장하지 않는다.
...
GitHub Pages에는 14일 반복 상승 정량 순위 200개 전체를 게시한다. 7·14·21일 탭은 동일 종목군과 동일한 1~200 순번을 유지하고, 각 기간의 실제 분봉 지표·수급과 상위 50개의 등급·코멘트만 변경한다.
...
화면은 하위 탭 없이 `정량 순위 200개` 종합표 하나로 구성한다. 종목명 또는 종목코드 검색창을 제공하며 기본 상태에서는 200개를 모두 보여준다. 종합표 한 행에서 기간별 차트·수급과 상위 50개의 뉴스·AI 코멘트를 함께 비교하며, 선택형 `추천 이상`, `박스 하단 35% 이하` 필터는 사용자가 원할 때만 적용한다. 외국인·기관 수급은 표와 AI 분석에서 참고하되 단순 합산 양수 여부로 후보를 숨기는 필터는 제공하지 않는다.
...
공개 정량 순위는 시총 상위 200개 전체를 14일 반복 상승 기준으로 고정한다. 상위 50개만 AI 심층검토하며 나머지 150개도 분봉·가격·수급 비교를 위해 숨기지 않는다. 이후 1·3·5거래일 성과, MAE, 비용 차감 수익을 저장해 반복 상승 순위와 AI 검토의 거짓양성·거짓음성을 측정한다.
