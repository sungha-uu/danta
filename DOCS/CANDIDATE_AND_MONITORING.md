...
- 미니차트를 누르면 선택 기간의 60분봉 확대 흐름을 표시한다. 팝업 헤더 첫 줄은 `종목명 · 현재가 ○○원 · 선택기간 60분봉`, 둘째 줄은 `평균 반복 상승폭·반복 횟수·평균 6% 도달시간·하단 방향`을 표시한다. 나머지 차트·범례·OHLCV 표 규칙은 유지한다.
...
- AI: 14일 상위 50개만 심층검토
- 현재 위치·하단 방향·과거 +10% 이력은 표시·AI·진입 참고값이며 정량 순위를 바꾸지 않음
