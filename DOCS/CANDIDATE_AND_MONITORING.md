...
동일 데이터와 `calculation_version`이면 동일한 적격 후보 수와 순위가 나와야 한다. 점수, 특징, 기준시각, 선정·감점·자격탈락 이유를 저장하며 미래 데이터를 사용하지 않는다.
...
GitHub Pages에는 자격 게이트를 통과한 공식 정량 후보만 최대 30개 게시한다. 정량 상위 50개 감사 풀은 비공개 운영 자료이며 공식 후보·AI 전수검토·집중 감시·`ENTRY_MANDATE` 대상이 아니다. AI는 자격 게이트 밖의 종목을 공식 후보로 추가할 수 없다.
...
화면은 별도 검색창이나 하위 탭 없이 `적격 후보 N개` 종합표 하나로 구성한다. 종합표 한 행에서 기간별 차트·수급·뉴스·AI 코멘트를 함께 비교하며, 선택형 `추천 이상`, `박스 하단 35% 이하` 필터만 제공한다. 외국인·기관 수급은 표와 AI 분석에서 참고하되 단순 합산 양수 여부로 후보를 숨기는 필터는 제공하지 않는다.
...
- 박스 하단 방향이 첫 거래일 대비 `-8%` 초과
- 박스 하단 접촉 뒤 해당 거래일을 포함한 3거래일 이내 `박스 하단 × 1.10`에 최소 1회 도달

적격 후보는 최대 30개이며 적격 수가 부족하면 부적격 종목으로 채우지 않는다. 정량 상위 50개는 공개 후보가 아니라 필터 성능 검증용 비공개 감사 풀로 유지한다. 감사 풀에는 탈락 사유, 이후 +10% 도달 여부, MAE, 비용 차감 수익을 저장해 거짓양성·거짓음성을 측정한다.
