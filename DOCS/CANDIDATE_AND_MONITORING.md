...
- 차트 팝업의 현재가 기준선은 상단·하단선보다 얇은 검은색 점선으로 표시한다. 차트 오른쪽에 전용 문구 여백을 두고 `상단(현재가+10%)`·`하단`·`현재가` 이름을 점선 종점 바깥에 선 색상으로 왼쪽 정렬한다. 가까운 선의 문구는 겹치지 않도록 위·아래로 배치한다.
...
- 미니차트를 누르면 선택 기간의 60분봉 확대 흐름을 표시한다. 팝업 헤더 첫 줄은 `종목명 · 현재가 ○○원 · 선택기간 60분봉`, 둘째 줄은 기존 `일중 진폭·저점 반등·목표 도달일수·하단 방향` 요약을 표시하며 기존 글자 크기를 유지한다. 팝업은 화면 높이를 최대한 활용하고 차트 높이는 일자별 표의 노출 행을 늘릴 수 있도록 제한한다. 확대 차트의 가로축에는 거래일, 세로축에는 원 단위 가격 눈금을 함께 표시한다. 기준선은 `상단(현재가+10%)`을 빨간 점선, `하단`을 파란 점선, `현재가`를 더 얇은 검은 점선으로 구분하고 이름은 점선 종점 오른쪽의 전용 여백에 표시한다. 팝업 표는 거래일을 세로 행, 09~15시를 가로 열로 배치하며 각 셀은 종가를 크게 표시하고 `량·시 / 저·고` 순서의 축약 OHLCV를 표시한다. 여기서 `시`는 시세가 아니라 해당 60분봉의 시가다. 날짜별 시간 흐름을 왼쪽에서 오른쪽으로 읽게 하며 브라우저에서 외부 시세 API를 호출하지 않는다.
