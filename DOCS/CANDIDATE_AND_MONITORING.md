...
- 내부 200개의 정량 구조를 계산해 14일 순위로 전부 공개한다. 이 가운데 14일 자격 게이트를 통과한 최대 30개만 공식 주문 후보로 표시한다. 7·14·21일 탭은 같은 200개 순서를 유지하며 해당 기간 지표만 변경한다.
...
| 주문 선택 | +10% 자격 게이트 통과 종목 중 최대 30개만 가능 |

```text
sort_key = (-repeat_count, -average_up_swing_pct, average_time_to_6pct, symbol)
```

동일 데이터와 `calculation_version`이면 동일한 200개 내부 계산군과 공식 후보 최대 30개가 나와야 한다. 점수, 특징, 기준시각과 자격 근거를 저장하며 미래 데이터를 사용하지 않는다.

## 5. AI 정량 상위 50개 등급

AI는 14일 정량 순위 상위 50개를 심층검토하며 자격 게이트 밖의 종목을 공식 주문 후보로 추가하거나 원시 숫자를 바꿀 수 없다. 다음을 검토한다.
...
GitHub Pages에는 시총 상위 200개를 모두 게시한다. 7·14·21일 탭은 동일한 14일 기준 순번 1~200을 유지하고 각 기간의 실제 분봉 지표·수급·등급·코멘트만 변경한다. 상위 50개에는 AI 심층검토 결과를 표시하고, +10% 자격 게이트를 통과한 최대 30개에만 선택 체크박스와 주문 승인문 생성 권한을 준다.
...
- 내부 계산: 시총 상위 200개 전체
- 공개·AI: 14일 자격 게이트 공식 후보 최대 30개 전체 심층검토
...
내부 정량 계산은 시총 상위 200개 전체를 대상으로 한다. 공개 보고서는 14일 자격 게이트를 통과한 최대 30개만 고정하고 AI가 전부 심층검토한다. 이후 1·3·5거래일 성과, MAE, 비용 차감 수익을 저장해 자격 게이트와 AI 검토의 거짓양성·거짓음성을 측정한다.
