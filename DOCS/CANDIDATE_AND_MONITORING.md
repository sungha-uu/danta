...
- 미니차트를 누르면 선택 기간의 60분봉 확대 흐름을 표시한다. 팝업 표는 거래일을 세로 행, 09~15시를 가로 열로 배치하고 각 셀에 해당 60분봉 OHLCV를 표시해 날짜별 시간 흐름을 왼쪽에서 오른쪽으로 읽게 한다. 브라우저에서 외부 시세 API를 호출하지 않는다.
...
### 10.1 활성 목표 버전: `intraday-elasticity-v4-multitarget`
...
- 초기 정량 가중치: 현재 가격 공간 25%, 하단 근접도 20%, 다중 목표 도달 빈도 20%, 유동성 15%, 저점 반등력 12%, 일중 진폭 8%, 하단 하락 추세 감점 최대 15점. 이미 상승한 고변동 종목보다 현재 하단에서 재상승 공간이 있는 종목을 우선한다.
