...
모의 자율매수의 진입 기회는 내부적으로 두 유형을 구분한다. 사용자 대시보드에
새 열을 임의로 추가하지 않고 등급 근거와 감사 데이터에만 유형을 기록한다.

- `LOWER_REVERSAL`: 현재가가 14일 기간 하단 35% 이내이고 하단 접촉 뒤 실제
  `+10%` 도달 이력이 있으면서 하단 기준 목표가가 아직 현재가 위에 있는 최초
  반등 후보
- `REPEAT_CONTINUATION`: 하단 기준 첫 `+10%`는 이미 통과했지만 14일 정량
  상위 20위, 6% 이상 비중복 상승 5회 이상, 평균 상승폭 10% 이상, 평균 6%
  도달 6거래시간 이내, 기간 고점 대비 8% 이상 가격 공간, 현재 위치 75% 이하,
  거래량 배율 0.75 이상이고 구조적 붕괴가 아닌 반복상승 지속 후보

지속형은 급등 추격 주문을 뜻하지 않는다. 보고서 현재가보다 비싸게 사지 않으며
로컬 진입 엔진의 하락 안정·매수세 회복·스프레드·시장 위험 게이트를 동일하게
통과해야 한다. 임계값은 `paper-continuation-entry-v1` 모의 개선판으로 기록하고
워크포워드 결과 없이 실계좌 기준으로 승격하지 않는다.

## 2. 투자 가능 종목군

- KOSPI 전체 종목 마스터에서 저비용 일별 자료로 시가총액 순위를 먼저 계산하고, 거래 가능한 보통주 상위 200개에 1분봉을 수집한다.
- KOSPI 보통주를 기본 대상으로 하며 우선주, ETF, ETN, 스팩은 별도 전략 버전 전까지 제외한다.
- 거래정지, 관리·투자경고·위험, 상장폐지 절차, 불성실공시 등 주문 위험 종목을 제외한다.
- 활성 수집 유니버스 `kospi-market-cap-top200-v1`은 최신 KRX 거래일의 시가총액 내림차순 상위 200개다. 재무상태와 거래대금은 200개 수집 여부를 바꾸지 않고 AI 위험·유동성 평가 자료로 사용한다.
- 재무상태의 정상 여부는 강제 제외 기준으로 사용하지 않고 AI 위험 검토 자료로 남긴다. 거래정지·관리·투자경고·상장폐지 절차 등 직접 주문 위험 상태는 계속 강제 제외한다.
...
- 상위 200개 구성과 시가총액·일평균 거래대금은 매 실행마다 `market-cap-top-200-v1` 스냅샷으로 저장한다. 신규 편입 종목은 현재 유니버스의 21거래일 기준 시작점까지 누락분을 따라잡고, 이탈 종목 자료는 재편입·감사를 위해 즉시 삭제하지 않는다.
...
사용자 화면과 진입 승인에 쓰는 `기간 최저가·기간 최고가`는 선택한
7·14·21거래일 원본 1분봉의 실제 저가 최솟값과 고가 최댓값이다. 분위값 기반
경계는 활성 박스 같은 내부 연구 산출물에만 사용할 수 있으며 실제 최저가·최고가라는
이름으로 노출하거나 주문 목표가에 사용할 수 없다.
...
현재 계산 버전은 `intraday-actual-10pct-gate-v14-visible-extrema-top200-official30-ai50-kospi-market-cap-top200-v1`이다.

### 3.0.1 반복 상승 구간

- 정렬용 원본은 시간순 1분봉 종가다. 저점에서 6% 이상 상승한 뒤 직전 고점에서 다시 6% 이상 하락해야 하나의 완료 상승 구간으로 확정한다. 보고서 끝에서 진행 중인 상승도 이미 6%에 도달했다면 1회로 기록하되 `OPEN`으로 구분한다.
- 같은 상승 경로의 6%·8%·10% 도달을 여러 번 세지 않는다. 6% 이상 하락해 새로운 저점 탐색 구간으로 전환된 뒤 다시 6% 상승해야 다음 횟수로 인정한다.
- 사용자 지표는 `평균 반복 상승폭`, `6% 이상 반복 상승 횟수`, `평균 6% 도달시간`이다. 시간은 실제 거래 분 수로 계산해 거래시간 단위로 표시한다.
- 1차 정량 순위는 가중합을 사용하지 않고 `반복 횟수 내림차순 → 평균 상승폭 내림차순 → 평균 도달시간 오름차순`으로 정렬한다. 유동성은 상위 200개 적격성 게이트로만 사용하고 순위 점수에 섞지 않는다.
- 내부 200개의 정량 구조를 계산해 14일 순위로 전부 공개하고 200개 모두를 공식 주문 후보로 표시한다. 14일 자격 게이트는 추천·위험 정보로만 유지한다. 7·14·21일 탭은 같은 200개 순서를 유지하며 해당 기간 지표만 변경한다.
...
`LOW` 신뢰도만으로 긍정적인 종합 의견을 단정하지 않는다. 현재가가 구조 무효화선 아래면 AI 코멘트에 위험과 관찰 조건을 명시한다.

활성 하단권·활성 상단권·재도달/손절·활성 신뢰와 `+10%` 도달 이력은 내부 진단과 워크포워드 연구 산출물에만 보존한다. 사용자 대시보드의 가격 구간 열은 `기간 최저가 → +10% 목표가 → 기간 최고가` 순서로 표시한다. 기존 `일중 진폭·저점 반등·도달일수` 열은 사용자 승인에 따라 `반복 상승폭·반복 횟수·도달시간`으로 교체한다.

과거 `+10%` 이력, 현재 위치와 하단 방향은 정량 순위의 선행 조건이나 가중치가 아니다. 대시보드 비교값과 AI·진입 단계의 참고값으로만 유지한다.

후보표의 `현재 위치`와 사용자가 승인하는 목표가는 선택한 7·14·21일 전체 원본
분봉의 실제 최저가·최고가로 계산한다. 과거 `+10%` 반복 자격은 오늘 새 최저가가
생겼다는 이유만으로 과거 반등 표본을 모두 지우지 않도록 60분봉 10분위 기반 내부
하단 접촉 사건을 사용한다. 이 내부 자격 경계는 실제 기간 최저가라는 이름으로
노출하지 않는다.

같은 보고서의 현재가는 선택 창 마지막 원본 분봉 종가여야 한다. 따라서 READY
창마다 반드시 `기간 최저가 ≤ 현재가 ≤ 기간 최고가`이고, 표시된 최저가·최고가는
각각 `chart_bars.low`의 최솟값과 `chart_bars.high`의 최댓값과 정확히 일치해야
한다. 하나라도 어기면 후보 산출·주문 승인·Pages 배포를 중단한다.
...
단타 후보의 핵심은 기간 전체 박스 폭 하나가 아니라 1분봉에서 `6% 이상 상승 구간이 얼마나 크고 자주, 빠르게 반복되었는가`다.

```text
up_swing_pct = (confirmed_peak_close / pivot_low_close - 1) × 100
repeat_count = count(non_overlapping up_swing_pct >= 6)
average_up_swing_pct = mean(up_swing_pct)
average_time_to_6pct = mean(first_6pct_reach_trading_minute - pivot_low_trading_minute)
current_vs_window_high_pct = (current_price / window_high - 1) × 100
lower_trend_pct = (last_daily_low / first_daily_low - 1) × 100
```

대시보드에는 1분봉으로 계산한 `평균 반복 상승폭`, `6% 이상 반복 상승 횟수`, `평균 6% 도달시간`을 표시한다. `기간고점 대비 현재가`는 선택 기간 최고가보다 현재가가 얼마나 낮은지를 음수·파란색으로 표시하며 손실률이나 예측 수익률이 아니다.

하단 방향·좋은 눌림·구조적 붕괴·뉴스·수급은 내부 1차 정렬을 바꾸지 않는다. 현재 위치 35% 이내와 실제 +10% 도달 이력은 공식 후보 자격에만 사용하며, 나머지 값은 공식 후보 전체 AI 심층검토와 사용자가 실제 진입 종목을 고르는 단계에서 해석한다.

## 4. 내부 정량·공식 주문 후보 200개

| 영역 | 특징 |
| --- | --- |
| 반복 상승 | 평균 상승폭, 6% 이상 비중복 횟수, 평균 6% 도달시간 |
| 가격 참고 | 기간 최저가, +10% 목표가, 기간 최고가, 현재 위치, 하단 방향 |
| 내부 계산군 | KOSPI 보통주 시가총액 상위 200개와 주문 가능 상태 |
| 공식 자격 | 현재 위치 35% 이내, 하단 접촉 후 3거래일 안에 실제 +10% 도달 |
| 공개 표시 | 시총 상위 200개 전체의 정량 데이터와 차트 |
| AI 종합 코멘트 | 14일 정량 순위 상위 50개의 수급·뉴스·공시·토론·재무 위험 설명 |
| 주문 선택 | 시가총액 상위 200개 모두 가능, 한 승인문에서는 최대 3개 |

```text
sort_key = (-repeat_count, -average_up_swing_pct, average_time_to_6pct, symbol)
```

동일 데이터와 `calculation_version`이면 동일한 200개 내부 계산군·공식 주문 후보가 나와야 한다. 점수, 특징, 기준시각과 추천 근거를 저장하며 미래 데이터를 사용하지 않는다.

## 5. AI 정량 상위 50개 종합 코멘트

AI는 14일 정량 순위 상위 50개를 심층검토하되 등급·점수·주문 자격을 만들지 않는다. 정량 원시값을 바꾸지 않고 다음 내용을 자연어 종합 코멘트로 설명한다.
...
따라서 `하단 추세 -N%`만으로 공식 후보를 탈락시키거나 AI 비추천으로 만들지 않는다. AI는 `고점 대비 현재 할인 폭 × 과거 단기 반복 상승 여력 × 당일 외국인·기관 자금 흐름`을 핵심으로 평가한다. 하락폭이 클수록 잠재 보상은 커질 수 있지만 자동 진입 확률이 높아지는 것은 아니다. 실제 진입은 최근 1분봉의 추가 하락 둔화와 매수세 회복을 확인하고, KIS 장중 외국인·기관 합산 추정값으로 대규모 자금 유출이 아닌지도 함께 점검한다. 순유입은 우대 신호지만 필수 통과 조건이 아니며 전일 보고서 수급은 장중 자금 유출을 대체할 수 없다.

장중 자율 후보는 손절 이력과 무관하게 `기간 고점 대비 할인 폭 40점 + 반복 상승
정량 순위 40점 + 당일 외국인·기관 수급 20점`으로 비교한다. 순유입은 가점,
약한 순유출은 감점이며 둘 다 다른 조건과 합산한다. 다만 수급강도 `-3.00%` 이하의
심각한 순유출은 신규진입을 차단한다. 이는 초기 안전 임계값이므로 실제 성과와
오탐·미탐을 추적해 버전으로 조정한다.

AI 출력에는 정량 상위 50개에 대한 종합 의견, 근거, 위험, 무효화 조건, 데이터 시각,
모델 ID와 프롬프트 버전을 포함한다. 코멘트는 `우선 검토할 만함`, `조건부 관찰이
적절함`, `현재 진입은 부담` 같은 설명을 사용할 수 있지만 이를 구조화된 등급이나
주문 게이트로 변환하지 않는다. 51~200위는 `AI 종합 코멘트 대상 외`로 표시하고,
AI 검토 실패는 `AI_COMMENT_MISSING`으로 기록한다.
...
에이전트 컨텍스트 검토판은 공식 후보 전체의 7·14·21일 반복 상승 구조, 외국인·기관 수급, 뉴스 감성, 토론 참고 신호와 Danta 독립 재무 위험 스냅샷을 결합한다. 외부 LLM이 연결되지 않은 단계에서는 `agent-context-review-v1`처럼 로컬·규칙 보조 검토임을 모델 버전에 명시하고 완전한 생성형 AI 검토로 가장하지 않는다.

21일 차트가 `원시세 복귀형 급등 소멸`에 해당하면 7·14·21일 모두 추천 자격을 제한한다. 이는 단순히 첫 종가와 현재가가 비슷하다는 이유만으로 정상 박스를 버리는 규칙이 아니다. `|첫 60분봉 종가 대비 현재가| ≤ 15%`, `기간 중 첫 종가 대비 최고가 급등 ≥ 25%`, `최고가가 기간 전반 50% 안에 출현`, `마지막 1/3 거래일의 일별 고가 80분위가 첫 1/3보다 10% 이상 하락`을 모두 만족할 때만 적용한다. 급등 뒤 상단 가격대가 단계적으로 낮아져 원래 가격대로 돌아온 경우를 뜻한다. 공개 비교군 50개에서는 삭제하지 않고 `비추천` 사유로 남겨 필터의 거짓양성을 감사하며, 사용자 승인 없이 별도 화면 열을 추가하지 않는다.

## 6. 후보 대시보드

GitHub Pages에는 시총 상위 200개를 모두 게시한다. 7·14·21일 탭은 동일한 14일 기준 순번 1~200을 유지하고 각 기간의 실제 분봉 지표·수급·종합 코멘트만 변경한다. AI 등급·AI 점수·추천 필터는 표시하지 않는다. 상위 50개에는 AI 종합 코멘트를 표시하고, 200개 모든 행에 선택 체크박스와 주문 승인문 생성 권한을 준다.

기본 화면:

- 종목명·코드·업종·현재가 뒤에 `기간수익률·기간고점 대비·가격 흐름·기간 최저가·+10% 목표가·기간 최고가·현재 위치·하단 방향·반복 상승폭·반복 횟수·도달시간` 순서로 배치하고, 이후 수급·점수·코멘트·뉴스를 표시
...
화면은 하위 탭 없이 공식 후보 종합표 하나로 구성한다. 종목명 또는 종목코드 검색창을 제공하며 기본 상태에서는 적격 후보를 모두 보여준다. 종합표 한 행에서 기간별 차트·수급·뉴스·AI 코멘트를 함께 비교하며, 선택형 `추천 이상`, `박스 하단 35% 이하` 필터는 사용자가 원할 때만 적용한다. 외국인·기관 수급은 표와 AI 분석에서 참고하되 단순 합산 양수 여부로 후보를 숨기는 필터는 제공하지 않는다.
...
수급 열은 선택 기간 숫자를 헤더에 반복하지 않고 `개인(순매수억)`, `외국인(순매수억)`, `기관(순매수억)`, `금융투자(순매수억)`, `연기금(순매수억)`, `외국인+기관 수급강도(%)`로 표시한다. 긴 수급강도 제목은 두 줄로 줄여 표 너비를 절약한다. 투자자별 순매수 금액은 정수여도 `66.0`처럼 소수점 한 자리를 항상 표시한다. 상단 기간 버튼을 바꾸면 헤더는 유지되고 값만 같은 기간 기준으로 동시에 전환한다.
...
- 단일 `정량 순위 200` 종합표에서 박스·수익률·미니차트·수급·AI 종합 코멘트·뉴스·토론·네이버 링크를 한 행에 제공하되 에이전트용 원시 유동성·위험 열은 숨김
...
- 미니차트를 누르면 선택 기간의 60분봉 확대 흐름을 표시한다. 팝업 헤더 첫 줄은 `종목명 · 현재가 ○○원 · 선택기간 60분봉`, 둘째 줄은 `평균 반복 상승폭·반복 횟수·평균 6% 도달시간·상승세`를 표시한다. 날짜×시간 표는 각 60분봉의 종가만 표시하고 거래량·시가·저가·고가는 사용자 화면에서 숨겨 셀 너비와 높이를 줄인다. 원본 OHLCV는 공개 DTO와 계산 근거에 그대로 보존한다.
...
### 10.1 활성 연구 버전: `intraday-repeat-rise-v11-top200-fixed14-v1`

- 원본: KRX 정규장 1분봉 OHLCV
- 순위 계산봉: 1분봉 종가
- 표시 분석봉: 60분봉 OHLCV
- 창: 7·14·21거래일, 기본 14일
- 최초 공식 보고서는 세 기간이 모두 READY일 때 생성
- 박스: 60분봉 저가 10분위 하단과 고가 90분위 상단
- 상승 탄력성: 6% ZigZag로 분리한 비중복 상승 구간의 횟수·평균 폭·평균 도달시간
- 순위: 14일 `반복 횟수 내림차순 → 평균 상승폭 내림차순 → 평균 도달시간 오름차순`
- 내부 계산: 시총 상위 200개 전체
- 공개·주문 선택: 시가총액 상위 200개 전체
- AI 심층검토: 14일 정량 순위 상위 50개
- 현재 위치·상승세·과거 +10% 이력은 표시·AI·진입 참고값이며 정량 순위를 바꾸지 않음
...
- 최초 공식 보고서는 `kospi-market-cap-top200-v1` 전체의 최근 21거래일 1분봉 백필을 완료한 뒤 만든다. 이후에는 매일 완성된 1거래일 증가분만 수집한다.
...
### 10.3 장중 200종목 증분 오버레이

- 16:00 보고서는 완결된 거래일의 공식 기준선이며 장중에는 덮어쓰지 않는다.
- KRX 정규장 동안 시가총액 상위 200개 전체의 최신 1분봉 한 페이지를 30분마다
  증분 수집한다. 직전 13개 완결 거래일과 당일 누적분을 합쳐 14일 반복 상승폭,
  반복 횟수, 도달시간, 현재 위치와 장중 순위를 다시 계산한다.
- 결과는 `private/<environment>/intraday_candidate_overlay.json`에 원자적으로
  저장한다. 관측 후 35분 이내이고 서로 다른 종목 180개 이상일 때만 신규매수
  선정 자료로 사용할 수 있다.
- 공개 단기 후보 Pages에는 계좌·주문정보를 제외한 같은 오버레이를 최대 1시간
  주기로 `intraday_overlay.json`에 게시한다. 브라우저는 이를 5분마다 확인해 14일
  현재가·순위·반복 상승 지표·가격 차트를 갱신하며, 실패하면 16:00 완결 보고서를
  그대로 유지한다. 수급·뉴스·공시·AI 코멘트는 다음 16:00 기준선까지 임의로
  실시간 값처럼 바꾸지 않는다.
- 실시간 오버레이는 200개 전수 정량 재평가다. 외부 AI를 30분마다 200개에 다시
  호출하지 않으며 AI 등급이나 코멘트 완성 여부로 자동 진입 검토군을 자르지 않는다.
  14일 반복 상승 정량 순위·기간 고점 대비 할인 폭·장중 자금 흐름을 조합한다.
- 08:30~09:00에는 전일 14일 정량 순위와 NXT 가격·체결·호가, KRX 예상체결가로
  사전 관찰군만 갱신한다. 이 시간에는 당일 외국인·기관 KRX 추정 수급이 없으므로
  이를 추정해 채우거나 신규매수 근거로 가장하지 않는다. 09:00 이후 KRX 1분봉과
  외국인·기관 장중 추정 수급이 준비된 뒤 최종 종목 지정과 주문 판단을 수행한다.
- 오버레이 수집 실패·저커버리지·노후화는 신규 배치만 차단하거나 직전 완결
  보고서+현재가 폴백을 사용한다. 보유 포지션의 손절·익절·체결대조는 영향을 받지
  않는다.
- 중앙 코어가 하나의 활성 주문 배치만 실행하는 동안에도 최신 오버레이 revision마다
  다음 후보를 계속 재선정하고 `autonomous_next_selection.json`에 대기 스냅샷으로
  저장한다. 종목 구성이 달라질 때만 선정·지정가 메일을 다시 보내 과도한 알림을
  막는다. 기존 배치가 전량 청산되고 계좌가 평탄하며 시장 위험이 정상일 때 최신
  대기 스냅샷을 다시 검증해 실제 주문 배치로 승격한다. 하루 최대 3배치다.
- 일부 종목만 청산된 상태에서 빈 슬롯을 즉시 주문으로 채우는 기능은 다중 배치 자금
  예약과 주문 멱등성 검증 전까지 금지하지만, 빈 슬롯용 후보 탐색 자체는 중단하지 않는다.
- 지정 후보 수는 현재 보유 포지션과 진행 주문을 포함한 동시 최대 3종목 한도에서
  남은 슬롯만큼으로 제한한다. 예를 들어 하이닉스 1종목을 보유 중이면 다음 지정
  후보는 2종목이며, 보유 종목 자체는 새 후보에서 제외한다.

### 10.4 다음 거래일 사용자 우선 종목

- 사용자는 다음 거래일에 우선 검토할 종목을 최대 3개까지 미리 지정할 수 있다.
  비공개 `autonomous_candidate_preference.json`에 거래일·종목코드·생성시각을
  원자적으로 저장하며 Git이나 공개 대시보드에는 게시하지 않는다.
- 해당 거래일 자율 후보 정렬에서 유효한 사용자 우선 종목을 먼저 배치하고 나머지
  슬롯은 `14일 장중 정량 기회점수 → 기간 고점 대비 할인 폭 → 장중 수급` 순서로 채운다. 사용자 지정
  순서가 여러 개면 그 순서를 유지한다.
- 우선 지정은 분봉 완성도·신선도,
  시장 위험, 계좌 평탄, 스프레드와 진입 엔진을 우회하지 않는다. 조건을 통과하지
  못하면 지정가나 주문을 강제로 만들지 않고 감사 로그에 요청·선정 여부를 남긴다.
- 우선 지정은 해당 거래일 하루만 유효하다. 자동 연장·물타기·특정 가격의 강제
  주문으로 해석하지 않는다. 변경·해제는 새 원자 파일로 교체하거나 삭제한다.
- 사용자가 특정 거래일에 종목의 `필수 포함`을 명시해도 시총 상위 200개 공식 후보, 14일 분봉·구조
  `READY`, 시장 안전, 계좌 무결성, 진입 시 매도압력·스프레드 조건은 그대로
  적용한다. 이 예외는 감사 로그에 `REQUIRE_INCLUDE`로 남기며 다른 종목으로
  전파하거나 다음 거래일로 연장하지 않는다.
...
AI 연결 전 정량 보고서를 생성할 수는 있지만 `model_id=quant-baseline-no-llm`으로 명시하고 AI 분석 완료로 간주하지 않는다. 이 단계의 4단계 등급과 코멘트는 화면·데이터 흐름 검증용 정량 기준선이다. 실제 AI 검토가 연결되면 14일 정량 순위 상위 50개를 검토하며 정량 원시값은 수정하지 않는다.

사용자 화면에는 AI 등급과 AI 점수를 만들거나 표시하지 않는다. 정량 산식은 정량 순위와 원시 지표로만 표현하고, 자동 AI 검토가 실제로 성공해 `model_id`와 입력 해시가 기록된 경우에만 별도의 `AI 종합 코멘트`를 표시한다. 이 코멘트는 주문 자격이나 필수 통과 기준이 아니다.
...
화면의 `상승세`는 기존 박스 하단 방향과 같은 값으로, 첫 거래일 저가 대비 마지막 거래일 저가의 변화율을 뜻한다. 자격 탈락 조건이 아니라 할인 폭과 하락 속도를 설명하는 특징값으로 유지한다. 하락 중 외국인·기관 순유입과 반등 경험이 동반되면 기회 점수에 사용하고, 수급 이탈과 목표 미도달이 동반되면 AI 위험 점수와 진입 대기 조건에 사용한다.

후보표의 사용자 정렬은 `상승세`, `반복 상승폭`, `반복 횟수`, `외국인+기관 수급강도`, `정량점수` 다섯 열에만 제공한다. 각 제목을 누를 때 `오름차순 → 내림차순 → 정렬 해제`로 순환하고, 값이 없는 행은 방향과 무관하게 뒤에 둔다. 활성 정렬 열은 다른 배경색과 방향 표시로 구분한다. 정렬 상태는 7·14·21일 탭 전환에도 유지하되 새 기간의 값으로 다시 정렬한다. `순위`를 누르면 다른 정렬을 모두 해제하고 14일 고정 공식 순위 1위부터 200위까지 복원한다.

내부 정량 계산과 공식 주문 선택은 시총 상위 200개 전체를 대상으로 한다. AI는 14일 정량 순위 상위 50개를 심층검토한다. 이후 등급과 관계없이 상위 50개 전체의 판정을 동결하고 1·3·5거래일 성과, MAE, 비용 차감 수익을 저장해 정량 추천 정보와 AI 검토의 거짓양성·거짓음성을 측정한다. 자동매수 검토군과 전수 평가군의 구분, 지표와 보수적 승격 규칙은 [`PROFITABILITY_IMPROVEMENT.md`](PROFITABILITY_IMPROVEMENT.md)의 `AI 추천 등급 전수 추적`을 따른다.
...
- 토론 제목은 비신뢰 참고 신호이며 정량 순위의 직접 가산 근거로 사용하지 않는다.
