...
활성 하단권·활성 상단권·재도달/손절·활성 신뢰는 내부 진단과 워크포워드 연구 산출물에만 보존한다. 사용자 대시보드의 1차 판단 표에는 다음 값만 직접 표시한다.

- 진입 기준가: 선택 기간 박스 하단
- `+10%` 목표가: `진입 기준가 × 1.10`
- 기간 최고가: 선택 기간 원본 1분봉의 실제 최고가
- `+10%` 이력: 박스 하단 접촉 뒤 3거래일 안에 목표가에 실제 도달한 횟수
